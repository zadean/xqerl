%% @author Zack
%% @doc @todo Add description to xqldb_exi.
...
read(Grammars, Bin, #{event_fun  := Fun,
                      user_state := Ust} = State) ->
...
% {local_name, 1}
xml_name_partition() ->
   #{bits => 2, next_id => 4,next_s => 4,
     values => #{0 => <<"base">>, 1 => <<"id">>,
                 2 => <<"lang">>, 3 => <<"space">> }}.

% {local_name, 2}
xsi_name_partition() ->   #{bits => 1, next_id => 2,next_s => 2, 
     values => #{0 => <<"nil">>,
                 1 => <<"type">> }}.

xevent_code_match_fun({B1,B2,B3},
                      [{{N},S1},{{_},S2},
                       {{_},S3},{{_},S4},
                       {{_},S5},{{_},S6},
                       {{_},S7},{{_},S8},
                       {{_},S9} | T], Bin) -> % happens often
...
built_in_grammars() ->
   ...
   DocContent      = [{{0},    {?START_ELEMENT_UNDEF, ?DocEnd}},
                      {{1,0,0},{?COMMENT,             ?DocContent}},
                      {{1,0,1},{?PROC_INST,           ?DocContent}}],
   DocEnd          = [{{0},    ?END_DOCUMENT},
                      {{1,0,0},{?COMMENT,             ?DocEnd}},
                      {{1,0,1},{?PROC_INST,           ?DocEnd}}],
   FragmentContent = [{{0},    {?START_ELEMENT_UNDEF, ?FragmentContent}},
                      {{1},    ?END_DOCUMENT},
                      {{2,0},  {?COMMENT,             ?FragmentContent}},
                      {{2,1},  {?PROC_INST,           ?FragmentContent}}],
   StartTagContent = [{{0,0},  ?END_ELEMENT_UNDEF},
   ...
   ElementContent  = [{{0},    ?END_ELEMENT_DEF},
...
add_new_element_grammar(Name,Production,Grammars) 
  when is_map_key({?StartTagContent,Name}, Grammars) ->
   #{Production := {_,ToPushCont}} = Grammars,
   Grammars#{
     Production := push_new_production({?START_ELEMENT_DEF(Name),
...
add_new_element_grammar(Name,Production,Grammars) ->
   ...
   Locals = if is_map_key({local,Name}, Grammars) ->
   ...
   Grammars#{
     ...
     {local,Name} => Locals % local string table
   }.

add_new_attribute_to_grammar(QName, {_,EQName} = CurrentState, Grammars) ->
   #{CurrentState := {_,ToPushCont}} = Grammars,
   NewProd = push_new_production({?ATTRIBUTE_DEF(QName),
                                  {?StartTagContent,EQName}},
                                 ToPushCont),
   Locals = if is_map_key({local,QName}, Grammars) ->
...
add_new_text_to_grammar({_,QName} = Production, Grammars) ->
   #{Production := {_,ToPushCont}} = Grammars,
   Grammars#{
     Production := push_new_production({?TEXT_DEF(QName),
...
event_namespace(Grammars, Stream, NextProd, Stack, #{event_fun  := Fun,
                                                     user_state := Ust} = State) ->
...
event_comment(Grammars, Stream, NextProd, Stack, #{event_fun  := Fun,
                                                   user_state := Ust} = State) ->
   {Value, Rest} = string(Stream),
   Event = {comment, Value}, % only Str
   NewUst = Fun(Event, Ust),
   read(Grammars,NextProd,Rest,Stack,State#{user_state := NewUst}).

event_text(Grammars, Stream, NextProd, Stack, #{event_fun  := Fun,
                                                user_state := Ust} = State) ->
   {Grammars1, Value, Rest} = read_value(Stream, NextProd, Grammars),
   Event = {text, Value}, % can be {Id,Str} or just Id
   NewUst = Fun(Event, Ust),
   read(Grammars1,NextProd,Rest,Stack,State#{user_state := NewUst}).

event_attribute(Grammars, Stream, NextProd, Stack, QName, QNameKey, #{event_fun  := Fun,
                                                                      user_state := Ust} = State) ->
   {Grammars1, Value, Rest} = read_value(Stream, {attribute,QNameKey}, Grammars),
   Event = {attribute, QName, Value}, % {{Ns, Ln, Px}, Id | {Id, Str}}
   NewUst = Fun(Event, Ust),
   read(Grammars1,NextProd,Rest,Stack,State#{user_state := NewUst}).

event_proc_inst(Grammars, Stream, NextProd, Stack, #{event_fun  := Fun,
                                                     user_state := Ust} = State) ->
...
event_start_element(Grammars, Stream, ?DocEnd, Stack, QName, QNameKey, #{event_fun  := Fun,
                                                                          user_state := Ust} = State) ->
...
event_start_element(Grammars, Stream, _NextProd, Stack, QName, QNameKey, #{event_fun  := Fun,
                                                                           user_state := Ust} = State) ->
...
event_end_element(Grammars, Stream, _NextProd, [?DocEnd], #{event_fun  := Fun,
                                                             user_state := Ust} = State) ->
   Event = end_element,
   NewUst = Fun(Event, Ust),
   read(Grammars,?DocEnd,Stream,[?DocEnd],State#{user_state := NewUst});
event_end_element(Grammars, Stream, _NextProd, Stack, #{event_fun  := Fun,
                                                        user_state := Ust} = State) ->
   [_|Stack1] = Stack,
   H = hd(Stack1),
   Event = end_element,
...
event_end_document(Stream, #{event_fun  := Fun,
                             user_state := Ust}) ->
   Event = end_document,
   NewUst = Fun(Event, Ust),
   {ok, NewUst, Stream}.
...
read(Grammars,CurrentState,Bin,Stack,State) ->
   ...
   case Run of
      ...
      {{?START_ELEMENT_UNDEF,?DocEnd}, Stream} -> % root element
         {Grammars1,QName0,R1} = qname(Grammars, Stream),
         QName = qname_key(QName0),
...
read_value(<<0:8,R/bitstring>>, {_,QName}, Grammars) -> % local hit
   PartKey = {local, QName},
   #{PartKey := #{bits := B,
                  values := V}} = Grammars,
...
read_value(<<1:8,R/bitstring>>, _, Grammars) -> % global hit
   #{global  := #{bits := B}} = Grammars,
   %#{global  := {B,_,_}} = Grammars,
   <<GlbId:B/integer,R1/bitstring>> = R,
   {Grammars, GlbId, R1};

read_value(Bin, {_,QName}, Grammars) -> % new string
   PartKey = {local, QName},
   #{global  := Global,
     PartKey := Local} = Grammars,
   {Len2,R} = unsigned_integer(Bin),
   {Str, R1} = string_1(R,Len2 - 2,<<>>),
   {GId, Global1} = add_string_to_gpartition(Global), % do not need the value
   %{GId, Global1} = add_string_to_partition(Global, Str),
   {_LId, Local1} = add_string_to_partition(Local, GId),
   {Grammars#{global  := Global1,
              PartKey := Local1}, {GId, Str}, R1}.
...
bit_width(N) when N =< 16#8000 -> 15; % after this log2 should be faster
bit_width(N) -> 
   erlang:ceil(math:log2(N)).
...
unsigned_integer(<<1:1,I1:7/integer,0:1,I2:7/integer,Rest/bitstring>>) ->
   Num = I1 bor (I2 bsl 7),
   {Num,Rest};
unsigned_integer(<<1:1,I1:7/integer,1:1,I2:7/integer,0:1,I3:7/integer,
                   Rest/bitstring>>) ->
   Num = I1 bor (I2 bsl 7) bor (I3 bsl 14),
   {Num,Rest};
unsigned_integer(<<1:1,I1:7/integer,1:1,I2:7/integer,1:1,I3:7/integer,
                   0:1,I4:7/integer,Rest/bitstring>>) ->
   Num = I1 bor (I2 bsl 7) bor (I3 bsl 14) bor (I4 bsl 21),
   {Num,Rest};
unsigned_integer(<<1:1,I1:7/integer,1:1,I2:7/integer,1:1,I3:7/integer,
                   1:1,I4:7/integer,0:1,I5:7/integer,Rest/bitstring>>) ->
   Num = I1 bor (I2 bsl 7) bor (I3 bsl 14) bor (I4 bsl 21) bor 
           (I5 bsl 28),
   {Num,Rest};
unsigned_integer(<<1:1,I1:7/integer,1:1,I2:7/integer,1:1,I3:7/integer,
                   1:1,I4:7/integer,1:1,I5:7/integer,0:1,I6:7/integer,
                   Rest/bitstring>>) ->
   Num = I1 bor (I2 bsl 7) bor (I3 bsl 14) bor (I4 bsl 21) bor 
           (I5 bsl 28) bor (I6 bsl 35),
   {Num,Rest};
unsigned_integer(<<1:1,I1:7/integer,1:1,I2:7/integer,1:1,I3:7/integer,
                   1:1,I4:7/integer,1:1,I5:7/integer,1:1,I6:7/integer,
                   0:1,I7:7/integer,Rest/bitstring>>) ->
   Num = I1 bor (I2 bsl 7) bor (I3 bsl 14) bor (I4 bsl 21) bor 
           (I5 bsl 28) bor (I6 bsl 35) bor (I7 bsl 42),
   {Num,Rest};
unsigned_integer(_) -> throw({error,big_num}).
...
qname(#{uri := #compact_partition{bits = N} = UriP} = G,Bin) -> 
   ...
   {G3,Px,R3} = case R2 of
                _ when P =< 1 -> % none or one possible
                   {G2,0,R2};
                <<0:P,R8/bitstring>> ->
                   {A2,B2} = string(R8),
...
add_string_to_compact_partition(#compact_partition{next_id = NextId,
                                                   values  = Values
                                                  }, String) ->
   NewNext = NextId + 1,
   BitWidth = bit_width(NewNext + 1),
   {NextId,
    #compact_partition{bits = BitWidth,
                       next_id = NewNext,
                       values  = Values#{NextId => String}}}.

add_string_to_partition(#{next_id := NextId,
                          next_s  := NextS,
                          bits    := BitWidth,
                          values  := Values}, String) 
  when NextId =:= NextS ->
   NewNext = NextId + 1,
   {NextId,#{bits    => BitWidth + 1,
             next_s  => NextS bsl 1,
             next_id => NewNext,
             values  => Values#{NextId => String}}}; % new map when changing bits
add_string_to_partition(#{next_id := NextId,
                          values  := Values} = Part, String) ->
   NewNext = NextId + 1,
   {NextId,Part#{next_id := NewNext,
                 values  := Values#{NextId => String}}}.
  
add_string_to_gpartition(#{next_id := NextId,
                           next_s  := NextS,
                           bits    := BitWidth})
  when NextId =:= NextS ->
   NewNext = NextId + 1,
   {NextId,#{bits    => BitWidth + 1,
             next_s  => NextS bsl 1,
             next_id => NewNext}}; % new map when changing bits
