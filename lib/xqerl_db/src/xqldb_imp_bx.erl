...
read_inf_tags(Bin,#{meta := #{up_to_date := UpToDate}} = Map) ->
   case read_token(Bin) of
      ...
      {<<"PATH">>,Rest} -> % TODO ignore and rebuild, no namespaces in it
         % get paths
...
read_namespaces(Bin) -> % TODO similar iterator as gb_trees
   ...
   Node = #{prefixes => token_set_to_pos_list(Prefixes),
            uris     => token_set_to_pos_list(Uris),
            index    => Sub
           },
   {Node,Rest6}.

read_namespaces(Bin,0,Acc) -> {lists:reverse(Acc),Bin};
...
read_path_node(Bin,false) ->
   ...
   {_,Rest2} = read_number(Rest1), % nibble one
   {ChildCount,Rest3} = read_number(Rest2),
   {_,Rest4} = read_double(Rest3), % nibble one
...
read_path_node(Bin,true) ->
   ...
   Node = if Kind == element, ChildCount > 0 -> % element
                (path_stats(Stats))#{name => Name,
                                     kind => Kind,
                                     children => Children};
...
read_path_node(Bin,0,Acc) -> {lists:reverse(Acc),Bin};
...
read_textual_index(TblF,TxtF,TxtLu,Pid,Action) ->
   ...
   DF = 
     fun(List,Pid2) ->
           ...
           UnPack = fun({not_inline,compressed, <<P:38/integer>>}) ->
...
read_doc_uris(PathF,Tbl,Txt,DocOffs,Pid) ->
   try PathF() of % file may not exist, if not, create what it would have had
      Paths ->
...
get_text_pointer(<<_:5,Kind:3,_:16,Text:5/binary,_:64/binary>>) 
   when Kind == 2;
        Kind == 4;
        Kind == 5 ->
   get_text(Text);
...
get_text(<<1:1,0:1,1:1,_:5,Rest/binary>>) -> 
   {string, << <<C>> || <<C>> <= Rest, C =/= 0 >> };
% inline integer
get_text(<<1:1,0:7,Rest:32/integer>>) -> {integer,Rest};
...
normalize_table_pages(#{used_pages := Used,
                        fpres := FPres}) ->
...
read_longs(Rest,0,Acc) -> {lists:reverse(Acc),Rest};
...
-define(READ_AHEAD,64). % try to limit pread calls

read_file_token(Fd,Pos) ->
   case file:pread(Fd, Pos, ?READ_AHEAD) of
      {ok,<<0:1,0:1,B:6,Tok:B/binary,_/binary>>} -> Tok;
      {ok,<<0:1,_:1,B:14,Tok:B/binary,_/binary>>} -> Tok;
      {ok,<<0:1,_:1,B:14,R/binary>>} ->
         {ok,Tok} = file:pread(Fd, Pos + ?READ_AHEAD, B - (?READ_AHEAD - 2)),
         <<R/binary,Tok/binary>>;
      {ok,<<_:1,0:1,B:30,Tok:B/binary,_/binary>>} -> Tok;
      {ok,<<_:1,0:1,B:30,R/binary>>} ->
         {ok,Tok} = file:pread(Fd, Pos + ?READ_AHEAD, B - (?READ_AHEAD - 3)),
         <<R/binary,Tok/binary>>;
      {ok,<<_,B:32/integer,Tok:B/binary,_/binary>>} -> Tok;
...
read_double(Bin) ->
   {Token,Rest} = read_token(Bin),
   try 
      binary_to_integer(Token) of
      F ->
         {erlang:float(F),Rest}
   catch 
      _:_ ->
         try 
            binary_to_float(Token) of
            F ->
               {F,Rest}
         catch 
            _:_ ->
...
meta_values([{<<"STORAGE">>,V}|T],Map) -> meta_values(T,Map#{storage => V});
meta_values([{<<"ISTORAGE">>,V}|T],Map) -> meta_values(T,Map#{version => V}); % 8.6
meta_values([{<<"FNAME">>,V}|T],Map) -> meta_values(T,Map#{original => V});
meta_values([{<<"FTSTOP">>,V}|T],Map) -> meta_values(T,Map#{fulltext_stopwords => V});
meta_values([{<<"FTLANG">>,V}|T],Map) -> meta_values(T,Map#{fulltext_language => V});
meta_values([{<<"SIZE">>,V}|T],Map) -> meta_values(T,Map#{node_count => binary_to_integer(V)});
meta_values([{<<"NDOCS">>,V}|T],Map) -> meta_values(T,Map#{document_count => binary_to_integer(V)});
meta_values([{<<"MAXLEN">>,V}|T],Map) -> meta_values(T,Map#{max_index_val_length => binary_to_integer(V)});
meta_values([{<<"MAXCATS">>,V}|T],Map) -> meta_values(T,Map#{max_index_vals => binary_to_integer(V)});
meta_values([{<<"LASTID">>,V}|T],Map) -> meta_values(T,Map#{last_id => binary_to_integer(V)});
meta_values([{<<"TIME">>,V}|T],Map) -> meta_values(T,Map#{created_time => erlang:convert_time_unit(binary_to_integer(V), millisecond, native)});
meta_values([{<<"FSIZE">>,V}|T],Map) -> meta_values(T,Map#{file_size => binary_to_integer(V)});
meta_values([{<<"FTDC">>,V}|T],Map) -> meta_values(T,Map#{fulltext_diaretics_removal => to_bool(V)});
meta_values([{<<"UPDINDEX">>,V}|T],Map) -> meta_values(T,Map#{auto_update => to_bool(V)});
meta_values([{<<"AUTOOPT">>,V}|T],Map) -> meta_values(T,Map#{auto_optimize => to_bool(V)});
meta_values([{<<"TXTINDEX">>,V}|T],Map) -> meta_values(T,Map#{text_index => to_bool(V)});
meta_values([{<<"ATVINDEX">>,V}|T],Map) -> meta_values(T,Map#{attribute_index => to_bool(V)});
meta_values([{<<"TOKINDEX">>,V}|T],Map) -> meta_values(T,Map#{token_index => to_bool(V)});
meta_values([{<<"FTXINDEX">>,V}|T],Map) -> meta_values(T,Map#{fulltext_index => to_bool(V)});
meta_values([{<<"TXTINC">>,V}|T],Map) -> meta_values(T,Map#{element_names_text_index => V});
meta_values([{<<"ATVINC">>,V}|T],Map) -> meta_values(T,Map#{attribute_names_text_index => V});
meta_values([{<<"TOKINC">>,V}|T],Map) -> meta_values(T,Map#{attribute_names_token_index => V});
meta_values([{<<"FTXINC">>,V}|T],Map) -> meta_values(T,Map#{names_fulltext_index => V});
meta_values([{<<"SPLITS">>,V}|T],Map) -> meta_values(T,Map#{index_split_size => binary_to_integer(V)});
meta_values([{<<"CRTTXT">>,V}|T],Map) -> meta_values(T,Map#{keep_text_index => to_bool(V)});
meta_values([{<<"CRTATV">>,V}|T],Map) -> meta_values(T,Map#{keep_attribute_index => to_bool(V)});
meta_values([{<<"CRTTOK">>,V}|T],Map) -> meta_values(T,Map#{keep_token_index => to_bool(V)});
meta_values([{<<"CRTFTX">>,V}|T],Map) -> meta_values(T,Map#{keep_fulltext_index => to_bool(V)});
meta_values([{<<"FTSTEM">>,V}|T],Map) -> meta_values(T,Map#{fulltext_stemming => to_bool(V)});
meta_values([{<<"FTCS">>,V}|T],Map) -> meta_values(T,Map#{fulltext_case_sensitive => to_bool(V)});
meta_values([{<<"UPTODATE">>,V}|T],Map) -> meta_values(T,Map#{up_to_date => to_bool(V)});
meta_values([],Map) -> Map.
...
-define(V30, 
        M = map_value_3(<<V1:1, V2:1, V3:1>>,Fun),
...
-define(V38, 
        M = map_value_4(<<V1:1, V2:1, V3:1>>,Fun),
        unpack_text(R,Rest,Fun,S - 1,<<Acc/binary,M>>)).
-define(V38n, 
        M = map_value_4(V,Fun),
        unpack_text(R,Rest,Fun,S - 1,<<Acc/binary,M>>)).
...
-define(V8, <<V>> = <<V1:1, V2:1, V3:1, V4:1,
                      V5:1, V6:1, V7:1, V8:1>>,
        M = map_value(V,Fun),
        unpack_text(R,Rest,Fun,S - 1,<<Acc/binary,M>>)).
-define(V8n,
...
% 
unpack_text(_,_,_,0,Acc) -> Acc;

unpack_text(<<>>,<<R:4/bitstring,V:3/bitstring,1:1,Rest/binary>>,Fun,S,Acc) -> ?V30n;
unpack_text(<<>>,<<R:3/bitstring,V:3/bitstring,2:2,Rest/binary>>,Fun,S,Acc) -> ?V38n;
unpack_text(<<>>,<<R:1/bitstring,V:4/bitstring,4:3,Rest/binary>>,Fun,S,Acc) -> ?V416n;
unpack_text(<<>>,<<V2:1,V3:1,V4:1,V5:1,8:4,R:7/bitstring,V1:1,Rest/binary>>,Fun,S,Acc) -> ?V532;
unpack_text(<<>>,<<V5:1,V6:1,V7:1,V8:1,0:4,R:4/bitstring,V1:1,V2:1,V3:1,V4:1,Rest/binary>>,Fun,S,Acc) -> ?V8;

unpack_text(<<1:1>>,<<R:5/bitstring,V:3/bitstring,Rest/binary>>,Fun,S,Acc) -> ?V30n;
unpack_text(<<0:1>>,<<R:4/bitstring,V:3/bitstring,1:1,Rest/binary>>,Fun,S,Acc) -> ?V38n;
unpack_text(<<0:1>>,<<R:2/bitstring,V:4/bitstring,2:2,Rest/binary>>,Fun,S,Acc) -> ?V416n;
unpack_text(<<0:1>>,<<V:5/bitstring,4:3,Rest/binary>>,Fun,S,Acc) -> R = <<>>, ?V532n;
unpack_text(<<0:1>>,<<V4:1,V5:1,V6:1,V7:1,V8:1,0:3,R:5/bitstring,V1:1,V2:1,V3:1,Rest/binary>>,Fun,S,Acc) -> ?V8;

unpack_text(<<V3:1,1:1>>,<<R:6/bitstring,V1:1,V2:1,Rest/binary>>,Fun,S,Acc) -> ?V30;
unpack_text(<<2:2>>,<<R:5/bitstring,V:3/bitstring,Rest/binary>>,Fun,S,Acc) -> ?V38n;
unpack_text(<<0:2>>,<<R:3/bitstring,V:4/bitstring,1:1,Rest/binary>>,Fun,S,Acc) -> ?V416n;
unpack_text(<<0:2>>,<<R:1/bitstring,V:5/bitstring,2:2,Rest/binary>>,Fun,S,Acc) -> ?V532n;
unpack_text(<<0:2>>,<<V3:1,V4:1,V5:1,V6:1,V7:1,V8:1,0:2,R:6/bitstring,V1:1,V2:1,Rest/binary>>,Fun,S,Acc) -> ?V8;

unpack_text(<<V2:1,V3:1,1:1>>,<<R:7/bitstring,V1:1,Rest/binary>>,Fun,S,Acc) -> ?V30;
unpack_text(<<V3:1,2:2>>,<<R:6/bitstring,V1:1,V2:1,Rest/binary>>,Fun,S,Acc) -> ?V38;
unpack_text(<<4:3>>,<<R:4/bitstring,V:4/bitstring,Rest/binary>>,Fun,S,Acc) -> ?V416n;
unpack_text(<<0:3>>,<<R:2/bitstring,V:5/bitstring,1:1,Rest/binary>>,Fun,S,Acc) -> ?V532n;
unpack_text(<<0:3>>,<<V2:1,V3:1,V4:1,V5:1,V6:1,V7:1,V8:1,0:1,R:7/bitstring,V1:1,Rest/binary>>,Fun,S,Acc) -> ?V8;

unpack_text(<<V:3/bitstring,1:1>>,<<Rest/binary>>,Fun,S,Acc) -> R = <<>>, ?V30n;
unpack_text(<<V2:1,V3:1,2:2>>,<<R:7/bitstring,V1:1,Rest/binary>>,Fun,S,Acc) -> ?V38;
unpack_text(<<V4:1,4:3>>,<<R:5/bitstring,V1:1,V2:1,V3:1,Rest/binary>>,Fun,S,Acc) -> ?V416;
unpack_text(<<8:4>>,<<R:3/bitstring,V:5/bitstring,Rest/binary>>,Fun,S,Acc) -> ?V532n;
unpack_text(<<0:4>>,<<V,Rest/binary>>,Fun,S,Acc) -> R = <<>>, ?V8n;

unpack_text(<<R:1/bitstring,V:3/bitstring,1:1>>,<<Rest/binary>>,Fun,S,Acc) -> ?V30n;
unpack_text(<<V:3/bitstring,2:2>>,<<Rest/binary>>,Fun,S,Acc) -> R = <<>>, ?V38n;
unpack_text(<<V3:1,V4:1,4:3>>,<<R:6/bitstring,V1:1,V2:1,Rest/binary>>,Fun,S,Acc) -> ?V416;
unpack_text(<<V5:1,8:4>>,<<R:4/bitstring,V1:1,V2:1,V3:1,V4:1,Rest/binary>>,Fun,S,Acc) -> ?V532;
unpack_text(<<V8:1,0:4>>,<<R:1/bitstring,V1:1,V2:1,V3:1,V4:1,V5:1,V6:1,V7:1,Rest/binary>>,Fun,S,Acc) -> ?V8;

unpack_text(<<R:2/bitstring,V:3/bitstring,1:1>>,<<Rest/binary>>,Fun,S,Acc) -> ?V30n;
unpack_text(<<R:1/bitstring,V:3/bitstring,2:2>>,<<Rest/binary>>,Fun,S,Acc) -> ?V38n;
unpack_text(<<V2:1,V3:1,V4:1,4:3>>,<<R:7/bitstring,V1:1,Rest/binary>>,Fun,S,Acc) -> ?V416;
unpack_text(<<V4:1,V5:1,8:4>>,<<R:5/bitstring,V1:1,V2:1,V3:1,Rest/binary>>,Fun,S,Acc) -> ?V532;
unpack_text(<<V7:1,V8:1,0:4>>,<<R:2/bitstring,V1:1,V2:1,V3:1,V4:1,V5:1,V6:1,Rest/binary>>,Fun,S,Acc) -> ?V8;

unpack_text(<<R:3/bitstring,V:3/bitstring,1:1>>,<<Rest/binary>>,Fun,S,Acc) -> ?V30n;
unpack_text(<<R:2/bitstring,V:3/bitstring,2:2>>,<<Rest/binary>>,Fun,S,Acc) -> ?V38n;
unpack_text(<<V:4/bitstring,4:3>>,<<Rest/binary>>,Fun,S,Acc) -> R = <<>>, ?V416n;
unpack_text(<<V3:1,V4:1,V5:1,8:4>>,<<R:6/bitstring,V1:1,V2:1,Rest/binary>>,Fun,S,Acc) -> ?V532;
unpack_text(<<V6:1,V7:1,V8:1,0:4>>,<<R:3/bitstring,V1:1,V2:1,V3:1,V4:1,V5:1,Rest/binary>>,Fun,S,Acc) -> ?V8;

unpack_text(<<R:2/bitstring,V:3/bitstring,4:3>>,<<Rest/binary>>,Fun,S,Acc) -> ?V30n; % special 1st run 8-bit
unpack_text(<<R:1/bitstring,V:3/bitstring,8:4>>,<<Rest/binary>>,Fun,S,Acc) -> ?V38n; % special 1st run 8-bit
unpack_text(<<V2:1,V3:1,V4:1,16:5>>,<<R:7/bitstring,V1:1,Rest/binary>>,Fun,S,Acc) -> ?V416; % special 1st run 8-bit
unpack_text(<<V4:1,V5:1,32:6>>,<<R:5/bitstring,V1:1,V2:1,V3:1,Rest/binary>>,Fun,S,Acc) -> ?V532; % special 1st run 8-bit
unpack_text(<<V7:1,V8:1,0:6>>,<<R:2/bitstring,V1:1,V2:1,V3:1,V4:1,V5:1,V6:1,Rest/binary>>,Fun,S,Acc) -> ?V8. % special 1st run 8-bit
