...
-spec connect(
        ...
        Pass :: xq_types:xs_string()) -> xq_types:xs_anyURI().
...
-spec execute(
        xq_types:context(), 
        xq_types:xs_anyURI(),
        xq_types:xs_string()) -> xq_types:xs_string().
...
-spec query(
        xq_types:context(), 
        xq_types:xs_anyURI(),
        xq_types:xs_string()) -> [xq_types:xq_item()] | [].
query(Ctx, ?uri(Id), Query) when is_binary(Query) ->
   Conn = get_connection_pid(Ctx, Id),
   Qid = case basexerl:query(Conn, Query) of
            {ok, Qid0} -> Qid0;
            {error,_} -> throw_error(query)
         end,
   Res = case basexerl:q_results(Conn, Qid) of
            {ok, Res0} -> Res0;
            {error,_} -> throw_error(query)
         end,               
...
-spec query(
        ...
        xq_types:xs_anyURI(),
        xq_types:xs_string(),
        xq_types:xq_map()) -> [xq_types:xq_item()] | [].
query(Ctx, Id, Query, []) -> query(Ctx, Id, Query);
...
cast_typed_string(#{namespaces := N}, V, T) 
   when T == 'xs:QName';
        T == 'xs:NOTATION' ->
...
item_string_type(Item) when is_binary(Item) -> {Item, <<"xs:string">>};
...
item_string_type(Item) when is_float(Item) -> 
   S1 = xqerl_types:cast_as(Item, 'xs:string'),
   {S1, <<"xs:double">>};
item_string_type(true) -> {<<"true">>, <<"xs:boolean">>};
item_string_type(false) -> {<<"false">>, <<"xs:boolean">>};
item_string_type(infinity) -> {<<"INF">>, <<"xs:double">>};
item_string_type(neg_infinity) -> {<<"-INF">>, <<"xs:double">>};
item_string_type(nan) -> {<<"NaN">>, <<"xs:double">>};
item_string_type(neg_zero) -> {<<"-0">>, <<"xs:double">>};
item_string_type(#xqAtomicValue{value = infinity}) -> {<<"INF">>, <<"xs:float">>};
item_string_type(#xqAtomicValue{value = neg_infinity}) -> {<<"-INF">>, <<"xs:float">>};
item_string_type(#xqAtomicValue{value = nan}) -> {<<"NaN">>, <<"xs:float">>};
item_string_type(#xqAtomicValue{value = neg_zero}) -> {<<"-0">>, <<"xs:float">>};
