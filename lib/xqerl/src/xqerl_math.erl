...
'acos'(_Ctx,#xqAtomicValue{value = neg_zero}) -> ?dbl(math:acos(0));
'acos'(_Ctx,#xqAtomicValue{value = nan}) -> ?dbl(nan);
'acos'(_Ctx,#xqAtomicValue{value = X}) when abs(X) > 1 -> ?dbl(nan);
...
'asin'(_Ctx,#xqAtomicValue{value = X} = A) when X == 0 -> A;
'asin'(_Ctx,#xqAtomicValue{value = neg_zero}) -> ?dbl(neg_zero);
'asin'(_Ctx,#xqAtomicValue{value = nan}) -> ?dbl(nan);
'asin'(_Ctx,#xqAtomicValue{value=X}) when abs(X) > 1 -> ?dbl(nan);
...
'atan'(_Ctx,#xqAtomicValue{value = V} = A) when V == 0 -> A;
'atan'(_Ctx,#xqAtomicValue{value = neg_zero}) -> ?dbl(neg_zero);
'atan'(_Ctx,#xqAtomicValue{value = nan}) -> ?dbl(nan);
'atan'(_Ctx,#xqAtomicValue{value = infinity}) -> ?dbl(math:pi() / 2);
'atan'(_Ctx,#xqAtomicValue{value = neg_infinity}) -> ?dbl(- math:pi() / 2);
...
'atan2'(_Ctx,X,[Y]) -> 'atan2'(_Ctx,X,Y);
% special values
'atan2'(_Ctx,#xqAtomicValue{value = neg_zero},#xqAtomicValue{value = V}) 
   when V == 0.0 -> ?dbl(neg_zero);
'atan2'(_Ctx,#xqAtomicValue{value = neg_zero},#xqAtomicValue{value = V}) 
   when V == -1.0 -> ?dbl(-math:pi());
'atan2'(_Ctx,#xqAtomicValue{value = V},#xqAtomicValue{value = neg_zero}) 
   when V == -1.0 -> ?dbl(math:atan2(-1.0, 0.0));
'atan2'(_Ctx,#xqAtomicValue{value = neg_zero},#xqAtomicValue{value = V}) 
   when V == 1.0 -> ?dbl(neg_zero);
'atan2'(_Ctx,#xqAtomicValue{value = V},#xqAtomicValue{value = neg_zero}) 
   when V == 0.0 -> ?dbl(math:pi());
'atan2'(_Ctx,#xqAtomicValue{value = neg_zero},
        #xqAtomicValue{value = neg_zero}) -> ?dbl(-math:pi());
...
'cos'(_Ctx,#xqAtomicValue{value = neg_zero}) -> ?dbl(1.0);
...
'exp'(_Ctx,#xqAtomicValue{value = neg_infinity}) -> ?dbl(0.0);
...
'exp10'(_Ctx,#xqAtomicValue{value = neg_infinity}) -> ?dbl(0.0);
...
'log'(_Ctx,#xqAtomicValue{value = neg_infinity}) -> ?dbl(nan);
'log'(_Ctx,#xqAtomicValue{value = X}) when X == 0 -> ?dbl(neg_infinity);
'log'(_Ctx,#xqAtomicValue{value = X}) when X < 0 -> ?dbl(nan);
...
'log10'(_Ctx,#xqAtomicValue{value = neg_infinity}) -> ?dbl(nan);
'log10'(_Ctx,#xqAtomicValue{value = X}) when X == 0 -> ?dbl(neg_infinity);
'log10'(_Ctx,#xqAtomicValue{value = X}) when X < 0 -> ?dbl(nan);
...
'pow'(_Ctx,X,[Y]) -> 'pow'(_Ctx,X,Y);
'pow'(_Ctx,#xqAtomicValue{value = infinity},#xqAtomicValue{value = Y}) 
   when Y == 0 -> ?dbl(1.0);
'pow'(_Ctx,#xqAtomicValue{value = neg_infinity},#xqAtomicValue{value = Y}) 
   when Y == 0 -> ?dbl(1.0);
'pow'(_Ctx,#xqAtomicValue{value = nan},#xqAtomicValue{value = Y}) 
   when Y == 0 -> ?dbl(1.0);
'pow'(_Ctx,#xqAtomicValue{value = X},#xqAtomicValue{value = Y}) 
   when X == 0, Y < 0 -> ?dbl(infinity); 
'pow'(_Ctx,#xqAtomicValue{value = neg_zero},#xqAtomicValue{value = Y}) 
   when Y < 0, trunc(Y) == Y ->
   if trunc(Y) rem 2 == -1 ->
         ?dbl(neg_infinity);
      true ->
...
'pow'(_Ctx,#xqAtomicValue{value = neg_zero},#xqAtomicValue{value = Y}) 
   when Y > 0 -> ?dbl(0.0);
'pow'(_Ctx,#xqAtomicValue{value = X},#xqAtomicValue{value = _Y}) 
   when abs(X) == 1 -> ?dbl(1.0);
'pow'(_Ctx,#xqAtomicValue{value = X},#xqAtomicValue{value = Y}) ->
...
'sin'(_Ctx,#xqAtomicValue{value = neg_infinity}) -> ?dbl(nan);
...
'sqrt'(_Ctx,#xqAtomicValue{value = neg_zero}) -> ?dbl(0.0);
'sqrt'(_Ctx,#xqAtomicValue{value = neg_infinity}) -> ?dbl(nan);
...
'tan'(_Ctx,#xqAtomicValue{value = neg_zero}) -> ?dbl(neg_zero);
