...
-export([
    static_serialization_option_map/2,
...
serialization_option_map(
    ...
    % each element can have namespaces...
    List = parse_xml_options(Ch),
    serialization_option_map(List, merge_namespaces(ENs, Namespaces));
serialization_option_map(Options, Namespaces) when is_map(Options), not is_map_key(nk, Options) ->
...
% should be decimal
type_check_mapped_vals(<<"html-version">>, Val) -> decimal_value(Val);
...
true_false_def([], Def) ->
    Def;
true_false_def(true, _) ->
    <<"yes">>;
true_false_def(false, _) ->
    <<"no">>;
...
string_value([]) -> <<>>;
string_value(Val) -> xqerl_types:string_value(Val).
...
validate1([{?OUTPUT("parameter-document"), Value} | T], #{base_uri := BaseUri} = State, parser) ->
    try
        ...
        MFun = fun
            (K, _V, Acc) when is_map_key(K, Acc) ->
...
validate1([{?OUTPUT("use-character-maps"), _Value} | _], _, parser) ->
    % not allowed in prolog, only param doc
    ?err('XQST0109');
...
) when not is_map_key('html-version', Opts) ->
    validate2(Opts#{
        version := 1.0,
        'html-version' => Vers
    });
validate2(#{version := Vers}) when Vers =/= 1.0 ->
    ?err('SESU0013');
validate2(#{
...
validate2(#{
    ...
    version := 1.0,
    method := Method
}) when Method == xml; Method == xhtml ->
    ?err('SEPM0010');
validate2(#{'omit-xml-declaration' := _} = Opts) ->
    Opts;
...
split_names_list([#qname{} | _] = Vals, _) ->
    Vals;
...
get_parser_namespaces() ->
    ...
    Def = #xqNamespace{
        namespace = xqerl_context:get_default_element_type_namespace(parser),
        prefix = <<>>
    },
    Nss1 = [
...
get_html_version(#xqAtomicValue{value = #xsDecimal{int = 5, scf = 0}}) ->
    5.0;
get_html_version(#xqAtomicValue{value = #xsDecimal{int = 4, scf = 0}}) ->
    4.0;
...
get_version(Value) ->
    case xqerl_lib:trim(Value) of
        <<"1.0">> -> 1.0;
        % html versions that will later be checked
        <<"5.0">> -> 5.0;
        <<"4.0">> -> 4.0;
        <<"1.1">> -> ?err('SESU0013');
        _ -> ?err('SESU0013')
...
normalize_xml_character_map_2([], Acc) ->
    Acc;
normalize_xml_character_map_2(_, _) ->
    ?err('SEPM0017').
...
normalize_xml_character_map_3([H | T], Map) ->
    case H of
        ...
        [{char, C}, {string, S}] -> normalize_xml_character_map_3(T, Map#{C => {C, S}});
        [{string, _}, {char, C}] when is_map_key(C, Map) -> ?err('SEPM0017');
        [{string, S}, {char, C}] -> normalize_xml_character_map_3(T, Map#{C => {C, S}})
    end;
normalize_xml_character_map_3([], Map) ->
    Map.
