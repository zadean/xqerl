...
add_statically_known_namespace(
    _,
...
add_statically_known_namespace(parser, Namespace, Prefix) ->
    Old =
        case erlang:get('statically-known-namespaces') of
            undefined ->
...
add_statically_known_namespace(Tab, Namespace, Prefix) ->
    Old =
        case get(Tab, 'statically-known-namespaces') of
            undefined ->
                dict:from_list(static_namespaces());
            N ->
                N
...
add_in_scope_variable(Tab, Value) ->
    Old =
        case get_in_scope_variables(Tab) of
            undefined ->
                [];
            N ->
                N
...
add_statically_known_function(Tab, Value) ->
    Old =
        case get_statically_known_functions(Tab) of
            undefined ->
                [];
            N ->
                N
...
add_statically_known_decimal_format(Tab, Value) ->
    Old =
        case get_statically_known_decimal_formats(Tab) of
            undefined ->
                [];
            N ->
                N
...
set_context_item(Ctx, #{nk := _} = CI, Pos, Size) when is_integer(Size) ->
...
add_available_collection(Tab, Name, Value) ->
    Old =
        case get_available_collections(Tab) of
            undefined ->
                gb_trees:empty();
            T ->
                T
...
add_default_static_values(Tab, RawCdt) ->
    ...
    set_default_language(Tab, #xqAtomicValue{
        type = 'xs:language',
        value = <<"en">>
    }),
...
get_module_exports(Imports) when is_list(Imports) ->
    ...
    lists:foldl(
        fun({Ns, _Px}, {FunsAcc, VarsAcc, PropsAcc}) ->
            try
