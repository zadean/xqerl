...
-functions([
    {{qname,?NS, ?PX,<<"address">>},
     {seqType, 'xs:string', one}, [], {'address', 1}, 0, []},
    {{qname,?NS, ?PX,<<"cookie">>},
     {seqType, 'xs:string', zero_or_one}, [], {'cookie', 2}, 1, [{seqType, 'xs:string', one}]},
    {{qname,?NS, ?PX,<<"cookie">>},
     {seqType, 'xs:string', one}, [], {'cookie', 3}, 2, [{seqType, 'xs:string', one}, {seqType, 'xs:string', one}]},
    {{qname,?NS, ?PX,<<"cookie-names">>},
     {seqType, 'xs:string', zero_or_many}, [], {'cookie_names', 1}, 0, []},
    {{qname,?NS, ?PX,<<"header">>},
     {seqType, 'xs:string', zero_or_one}, [], {'header', 2}, 1, [{seqType, 'xs:string', one}]},
    {{qname,?NS, ?PX,<<"header">>},
     {seqType, 'xs:string', one}, [], {'header', 3}, 2, [{seqType, 'xs:string', one}, {seqType, 'xs:string', one}]},
    {{qname,?NS, ?PX,<<"header-names">>},
     {seqType, 'xs:string', zero_or_many}, [], {'header_names', 1}, 0, []},
    {{qname,?NS, ?PX,<<"hostname">>},
     {seqType, 'xs:string', one}, [], {'hostname', 1}, 0, []},
    {{qname,?NS, ?PX,<<"method">>},
     {seqType, 'xs:string', one}, [], {'method', 1}, 0, []},
    {{qname,?NS, ?PX,<<"parameter">>},
     {seqType, 'xs:string', zero_or_many}, [], {'parameter', 2}, 1, [{seqType, 'xs:string', one}]},
    {{qname,?NS, ?PX,<<"parameter">>},
     {seqType, 'xs:string', zero_or_many}, [], {'parameter', 3}, 2, [{seqType, 'xs:string', one}, {seqType, 'xs:string', one}]},
    {{qname,?NS, ?PX,<<"parameter-names">>},
     {seqType, 'xs:string', zero_or_many}, [], {'parameter_names', 1}, 0, []},
    {{qname,?NS, ?PX,<<"path">>},
     {seqType, 'xs:string', one}, [], {'path', 1}, 0, []},
    {{qname,?NS, ?PX,<<"port">>},
     {seqType, 'xs:integer', one}, [], {'port', 1}, 0, []},
    {{qname,?NS, ?PX,<<"query">>},
     {seqType, 'xs:string', zero_or_one}, [], {'query', 1}, 0, []},
    {{qname,?NS, ?PX,<<"remote-address">>},
     {seqType, 'xs:string', one}, [], {'remote_address', 1}, 0, []},
    {{qname,?NS, ?PX,<<"remote-hostname">>},
     {seqType, 'xs:string', one}, [], {'remote_hostname', 1}, 0, []},
    {{qname,?NS, ?PX,<<"remote-port">>},
     {seqType, 'xs:integer', one}, [], {'remote_port', 1}, 0, []},
    {{qname,?NS, ?PX,<<"scheme">>},
     {seqType, 'xs:string', one}, [], {'scheme', 1}, 0, []},
    {{qname,?NS, ?PX,<<"uri">>},
     {seqType, 'xs:anyURI', one}, [], {'uri', 1}, 0, []}
]).
...
remote_hostname(_) -> no_context_error().
...
remote_address(#{restxq_ctx := Req}) ->
    {IpAddress, _Port} = cowboy_req:peer(Req),
    iolist_to_binary(inet:ntoa(IpAddress));
remote_address(_) -> no_context_error().
...
remote_port(#{restxq_ctx := Req}) ->
    ...
    Port;
remote_port(_) -> no_context_error().
...
parameter_names(#{restxq_ctx := Req}) ->
    ...
    [K || {K, _} <- ParsedQs];
parameter_names(_) -> no_context_error().
...
parameter(_, _) -> no_context_error().
...
parameter(#{restxq_ctx := Req}, Param, Default) when is_binary(Param),
                                                     is_binary(Default) ->
    ParsedQs = cowboy_req:parse_qs(Req),
    Found = proplists:get_all_values(Param, ParsedQs),
    case Found of
...
parameter(_, _, _) -> no_context_error().
...
header_names(#{restxq_ctx := Req}) ->
    ...
    maps:keys(ParsedQs);
header_names(_) -> no_context_error().
...
%%  is returned instead.
header(#{restxq_ctx := Req}, Name, Default) when is_binary(Name),
                                                 is_binary(Default) ->
...
cookie_names(#{restxq_ctx := Req}) ->
    ...
    [K || {K, _} <- ParsedCs];
cookie_names(_) -> no_context_error().
...
%%  returned instead.
cookie(#{restxq_ctx := Req}, Name, Default) when is_binary(Name),
                                                 is_binary(Default) ->
    ParsedCs = cowboy_req:parse_cookies(Req),
    case lists:keyfind(Name, 1, ParsedCs) of
        {_, Token} -> Token;
...
cookie(#{restxq_ctx := _} = Ctx, Name, Default) ->
    cookie(
      ...
      xqerl_types:cast_as(Name, 'xs:string'),
      xqerl_types:cast_as(Default, 'xs:string'));
cookie(_, _, _) -> no_context_error().

-define(Q(V), #xqAtomicValue{type = 'xs:QName', 
                             value = #qname{namespace = ?ERRNS,prefix = ?ERRPX,
                              local_name = V}}).

no_context_error() ->
    E = #xqError{description = <<"Not in an HTTP context.">>,
                 name = ?Q(<<"REDY0001">>)},
    throw(E).
