...
receive_(_, _, Timeout) when is_integer(Timeout), Timeout < 1 ->
    do_throw(neg_timeout);
receive_(_, FromAddress, Timeout) when is_pid(FromAddress), is_integer(Timeout), Timeout > 0 ->
    receive
        #{
            from := FromAddress,
            value := Items
...
do_throw(bad_arity) ->
    E = #xqError{
        description = <<"Bad arity of spawned function.">>,
        name = ?Q(<<"bad-arity">>)
    },
    exit(E);
do_throw(bad_pid) ->
    E = #xqError{
        description = <<"Invalid actor address.">>,
        name = ?Q(<<"invalid-address">>)
    },
    exit(E);
do_throw(timeout) ->
    E = #xqError{
        description = <<"Receive timed out.">>,
        name = ?Q(<<"timout">>)
    },
    exit(E).
