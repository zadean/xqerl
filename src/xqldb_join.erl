...
lm(I1, I2, Acc) when is_function(I1), is_function(I2) -> lm(I1(), I2(), Acc);
lm(I1, I2, Acc) when is_function(I1) -> lm(I1(), I2, Acc);
lm(I1, I2, Acc) when is_function(I2) -> lm(I1, I2(), Acc);
lm([], [], Acc) ->
   lists:reverse(Acc);
lm([], Rest, Acc) ->
...
inter([H1|T1],[_H2|T2],Acc) ->
   inter(T1,T2,[H1|Acc]);
inter(R1, R2, Acc) ->% empty or con-fun
   {R1, R2, Acc}.
...
lu(I1, I2, Acc) when is_function(I1), is_function(I2) -> lu(I1(), I2(), Acc);
lu(I1, I2, Acc) when is_function(I1) -> lu(I1(), I2, Acc);
lu(I1, I2, Acc) when is_function(I2) -> lu(I1, I2(), Acc);
lu([],[],Acc) ->
   lists:reverse(Acc);
lu([],Rest,Acc) ->
...
union([H1|T1],[_|T2],Acc) -> %when H1 == H2 ->
   union(T1,T2,[H1|Acc]);
union(R1, R2, Acc) ->% empty or con-fun
   {R1, R2, Acc}.
