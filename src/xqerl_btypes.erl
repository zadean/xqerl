...
sequence_type([H|T],ListType) ->
   sequence_type(T,ListType band get_type(H)).


% is subtype
is_promotable(Type,TargetType) when Type band TargetType =:= TargetType -> true;
% is in union type
is_promotable(Type,TargetType) when TargetType band 1024 =:= 1024 andalso
                                    Type band TargetType =/= ?anyAtomicType andalso
                                    Type band TargetType =/= ?item -> true;
is_promotable(_Type,_TargetType) -> false.
...
get_type('xs:anyType'              ) ->                     1 ; % ignoring complex types for now, only used in elements
get_type('xs:untyped'              ) ->                     2 ; % ignoring complex types for now, only used in elements
get_type('xs:anySimpleType'        ) ->  10376293541461622784 ; % handling list and union types as anyAtomicType
...
get_type(                   1 ) ->  'xs:anyType'              ; % ignoring complex types for now, only used in elements
get_type(                   2 ) ->  'xs:untyped'              ; % ignoring complex types for now, only used in elements
