...
'acos'(_Ctx, nan) ->
    ?dbl(nan);
'acos'(_Ctx, X) when is_number(X), abs(X) > 1 ->
    ?dbl(nan);
'acos'(_Ctx, X) ->
...
'asin'(_Ctx, #xqAtomicValue{value = X}) ->
    'asin'(_Ctx, X);
'asin'(_Ctx, A) when A == 0 ->
    A;
'asin'(_Ctx, neg_zero) ->
    ?dbl(neg_zero);
'asin'(_Ctx, nan) ->
    ?dbl(nan);
'asin'(_Ctx, X) when abs(X) > 1 ->
    ?dbl(nan);
'asin'(_Ctx, X) ->
...
'atan2'(_Ctx, X, [Y]) ->
    'atan2'(_Ctx, X, Y);
'atan2'(_Ctx, #xqAtomicValue{value = X}, Y) ->
    'atan2'(_Ctx, X, Y);
'atan2'(_Ctx, X, #xqAtomicValue{value = Y}) ->
    'atan2'(_Ctx, X, Y);
% special values
'atan2'(_Ctx, neg_zero, V) when V == 0.0 ->
    ?dbl(neg_zero);
'atan2'(_Ctx, neg_zero, V) when V == -1.0 ->
    ?dbl(-math:pi());
'atan2'(_Ctx, V, neg_zero) when V == -1.0 ->
    ?dbl(math:atan2(-1.0, 0.0));
'atan2'(_Ctx, neg_zero, V) when V == 1.0 ->
    ?dbl(neg_zero);
'atan2'(_Ctx, V, neg_zero) when V == 0.0 ->
    ?dbl(math:pi());
'atan2'(_Ctx, neg_zero, neg_zero) ->
    ?dbl(-math:pi());
'atan2'(_Ctx, X, Y) ->
...
'exp10'(_Ctx, nan = X) ->
    X;
'exp10'(_Ctx, infinity = X) ->
    X;
'exp10'(_Ctx, neg_infinity) ->
    ?dbl(0.0);
'exp10'(_Ctx, X) ->
...
'log10'(_Ctx, nan = X) ->
    X;
'log10'(_Ctx, infinity = X) ->
    X;
'log10'(_Ctx, neg_infinity) ->
    ?dbl(nan);
'log10'(_Ctx, X) when X == 0 ->
    ?dbl(neg_infinity);
'log10'(_Ctx, X) when X < 0 ->
    ?dbl(nan);
'log10'(_Ctx, X) ->
...
'pow'(_Ctx, X, [Y]) ->
    'pow'(_Ctx, X, Y);
'pow'(_Ctx, #xqAtomicValue{value = X}, Y) ->
    'pow'(_Ctx, X, Y);
'pow'(_Ctx, X, #xqAtomicValue{value = Y}) ->
    'pow'(_Ctx, X, Y);
'pow'(_Ctx, infinity, Y) when Y == 0 ->
    ?dbl(1.0);
'pow'(_Ctx, neg_infinity, Y) when Y == 0 ->
    ?dbl(1.0);
'pow'(_Ctx, nan, Y) when Y == 0 ->
    ?dbl(1.0);
'pow'(_Ctx, X, Y) when X == 0, Y < 0 ->
    ?dbl(infinity);
...
'sqrt'(_Ctx, nan = X) ->
    X;
'sqrt'(_Ctx, infinity = X) ->
    X;
'sqrt'(_Ctx, neg_zero = X) ->
    X;
'sqrt'(_Ctx, neg_infinity) ->
    ?dbl(nan);
'sqrt'(_Ctx, X) ->
