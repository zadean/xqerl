...
expand_local_funs(
    ...
    OnlyPriv =
        ...
        fun
            ({_, _, Annos, {F, A}, _, _} = S) when ModType == library ->
...
init_fun_abs(Ctx0, KeysToAdd) ->
    ...
    Fun = fun
        ({context_item_type, _}, M) -> M;
        (Name, M) -> add_context_key(M, Name, Ctx)
    end,
...
add_context_key(Map, base_uri, #{'base-uri' := K}) ->
    Map#{'base-uri' => K};
add_context_key(Map, known_collations, #{
    'base-uri' := B,
    'default-collation' := D,
    known_collations := K
}) ->
...
add_context_key(Map, known_namespaces, #{
    'copy-namespaces' := C,
    namespaces := N
}) ->
    Map#{
...
add_context_key(Map, named_functions, #{
    'base-uri' := B,
    'construction-mode' := M,
    'copy-namespaces' := C,
    'default-collation' := D,
...
scan_mod(
    ...
    {ModNs, library, ImportNss, translate_variables(P1), translate_functions(P2), revert_all(P10),
        RestWrappers};
scan_mod(
    #xqModule{
        prolog = #xqProlog{
    ...
    % this will also setup the global variable match
    P4 = body_function(EmptyMap, Body, ImportNss, ContextTypes),
    ...
    ok = delete_global_funs(),
    P2 = lists:flatten([
        ?P(?LINE, export_functions(Functions)),
        %RestExports,
        [],
        ?Q(used_records())
    ]),
    AllAbs = lists:flatten([P1, [P1a | P2], P3, P4, P5, P6, P7, local_error_fun()]),
    {FileName, main, ImportNss, translate_variables(scan_variables(EmptyMap, Variables, public)),
...
init_function(ModName, Variables, Prolog1) ->
    ...
    VarSetFun =
        %%% mapfold here to make new ctx for each variable
        fun({_N, _T, _A, V, _Ext}, CtxVar) ->
    ...
    Clauses = [
        {clause, ?LINE, [{var, ?LINE, 'Ctx'}],
            [
                [
                    {call, ?LINE, {remote, ?LINE, {atom, ?LINE, erlang}, {atom, ?LINE, is_map_key}},
...
param_fields([{body, VarName} | T], Map) ->
    ...
    [
        ?P(?LINE, [
            "_@TmpAtom = xqerl_mod_http_client:read_body(Req),",
...
param_fields([{Atom, VarName} | T], Map) ->
    ...
    [
        ?P(?LINE, [
            "_@TmpAtom = cowboy_req:binding(_@Atom@, Req),",
...
param_headers(Params, Map) ->
    [
        begin
            ...
            TmpAtom = {var, ?LINE, list_to_atom("TVar_" ++ integer_to_list(VarId))},
            ?P(?LINE, [
                "_@TmpAtom = cowboy_req:header(_@ParamName@, Req, _@Default0@),",
                "_@VarAtom = xqerl_types:cast_as(_@TmpAtom, _@VarType1@)"
            ])
        end
    ...
    [
        ...
        | [
            begin
                ...
                TmpAtom = {var, ?LINE, list_to_atom("TVar_" ++ integer_to_list(VarId))},
                ?P(?LINE, [
                    "_@TmpAtom = proplists:get_value(_@CookieName@, Cookies, _@Default0@),",
                    "_@VarAtom = xqerl_types:cast_as(_@TmpAtom, _@VarType1@)"
                ])
            end
    ...
    [
        ...
        | [
            begin
                ...
                TmpAtom = {var, ?LINE, list_to_atom("TVar_" ++ integer_to_list(VarId))},
                ?P(?LINE, [
                    "_@TmpAtom = proplists:get_value(_@ParamName@, FormKeyVals, _@Default0@),",
                    "_@VarAtom = xqerl_types:cast_as(_@TmpAtom, _@VarType1@)"
                ])
            end
            || {ParamName, VarName, Default0} <- Params
    ...
    [
        ...
        | [
            begin
                ...
                TmpAtom = {var, ?LINE, list_to_atom("TVar_" ++ integer_to_list(VarId))},
                ?P(?LINE, [
                    "_@TmpAtom = proplists:get_value(_@ParamName@, QueryKeyVals, _@Default0@),",
                    "_@VarAtom = xqerl_types:cast_as(_@TmpAtom, _@VarType1@)"
                ])
            end
            || {ParamName, VarName, Default0} <- Params
...
export_atts([]) -> [];
export_atts([{F, A} | T]) -> lists:concat(["-export(['", F, "'/", A, "]). ", export_atts(T)]).
...
expr_do(Ctx, [T]) ->
    expr_do(Ctx, T);
expr_do(_Ctx, V) when is_number(V); is_binary(V) ->
    ?Q("_@V@");
expr_do(_Ctx, Atom) when
    Atom =:= infinity;
    Atom =:= neg_infinity;
    Atom =:= neg_zero;
    Atom =:= nan;
...
expr_do(Ctx, {'context-item', Line}) ->
    ...
    ?P(Line, [
        "case catch xqerl_context:get_context_item(_@CtxName) of",
...
expr_do(Ctx, {false_not_empty, Line, Expr}) ->
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_types:ensure_empty(_@E1) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
...
expr_do(Ctx, {variable, {Name, 1}}) when is_atom(Name) ->
    ...
    Result = var(?LINE, next_var_name()),
    ?Q([
        "case maps:get('@Name@',_@M, undefined) of",
...
expr_do(Ctx, #xqTypingExpr{
    ...
    ?P(L, [
        "case xqerl_seq3:'@FunName@'(_@Expr1) of",
...
expr_do(Ctx, #xqTypingExpr{
    ...
    Result = var(L, next_var_name()),
    ?P(L, [
        "case catch xqerl_types:cast_as(_@Expr1,'xs:QName', maps:get(namespaces, _@CtxVar)) of",
...
expr_do(Ctx, #xqTypingExpr{
    ...
    expr = Expr,
    type = Type
}) when Kind == cast; Kind == check; Kind == promote ->
    Expr1 = expr_do(Ctx, Expr),
    Type1 = expr_do(Ctx, Type),
    Error = var(L, next_var_name()),
    Result = var(L, next_var_name()),
    case Kind of
        cast ->
            ?P(L, [
                "case catch xqerl_types:cast_as(_@Expr1, _@Type1) of",
                " #xqError{} = _@Error -> local_error(_@Error, _@L@);",
                " _@Result -> _@Result",
                "end"
            ]);
        check ->
            ?P(L, [
                "case catch xqerl_types:check(_@Expr1, _@Type1) of",
                " #xqError{} = _@Error -> local_error(_@Error, _@L@);",
                " _@Result -> _@Result",
                "end"
            ]);
        promote ->
            ?P(L, [
                "case catch xqerl_types:promote(_@Expr1, _@Type1) of",
                " #xqError{} = _@Error -> local_error(_@Error, _@L@);",
                " _@Result -> _@Result",
                "end"
            ])
...
expr_do(Ctx, #xqSimpleMap{anno = Line, lhs = SeqExpr, rhs = MapExpr}) ->
    ...
    ?P(Line, [
        "case catch xqerl_seq3:map(_@CtxVar,_@FunAbs,_@SeqAbs) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
...
expr_do(Ctx, #xqRange{anno = Line, from = Expr1, to = Expr2}) ->
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_seq3:range(_@E1, _@E2) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
% context item declaration
expr_do(_Ctx, #xqContextItemDecl{
    anno = Line,
    default = undefined
}) ->
    {nil, Line};
expr_do(Ctx, #xqContextItemDecl{
    anno = Line,
    default = Expr
}) ->
    Expr1 = expr_do(Ctx, Expr),
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch begin _@Expr1 end of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
...
expr_do(Ctx, #xqMapConst{
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_mod_map:construct(_@CtxVar,_@@AVals) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
...
expr_do(Ctx, #xqSequence{vals = List}) ->
    Fun = fun
        ('empty-sequence') -> [];
        (#xqSequence{vals = SList}) -> alist(expr_do(Ctx, SList));
        (E) -> alist(expr_do(Ctx, E))
    end,
    Exprs = lists:flatmap(Fun, List),
    from_list_to_seq(Exprs);
expr_do(Ctx, #xqIfExpr{
    anno = Line,
    condition = If,
    if_true = Then,
    if_false = Else
    ...
    % catch this, can throw error
    ?P(Line, [
        "case catch xqerl_seq3:singleton_value(_@IfSt) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " true -> _@True; ",
        " false -> _@False",
        " end"
...
expr_do(Ctx, #xqPostfixExpr{
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_operators:lookup(_@CtxVar,_@MapExpr,all) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
expr_do(Ctx, #xqPostfixExpr{
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_operators:lookup(_@CtxVar,_@MapExpr,_@ValExp) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
expr_do(Ctx, #xqLookup{anno = Line, key = Key}) when Key == all; Key == wildcard ->
    CtxVar = var(Line, get_context_variable_name(Ctx)),
...
expr_do(
    #{context_variable := _} = Ctx,
    ?CALL(
        #xqFunctionDef{
            params = [A],
            body = {xqerl_mod_fn, lang, 2}
        } = FC,
        Line
    )
) ->
...
expr_do(
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_mod_fn:error(_@CtxName,_@@NewArgs) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
...
expr_do(
    ...
    ?P(Line, [
        "begin _@Ctx1Name = _@CtxName#{parent => erlang:self()}, ",
...
expr_do(
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch '@M@':'@F@'(_@CtxName,_@@NewArgs) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
...
expr_do(
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch '@F@'(_@CtxName,_@@NewArgs) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
...
expr_do(_Ctx, #xqFunctionDef{
    ...
    _ = add_used_record_type(xqFunction),
    ?P(Line, [
        "#xqFunction{annotations = _@Annos1@,",
        "            name = _@Name1@,",
        "            arity = _@Ay@,",
        "            params = _@Params1@,",
        "            type = _@Type1@,",
...
expr_do(Ctx, #xqComparisonExpr{
    ...
    E3 =
        if
            ...
            Op =:= 'eq' ->
                ?Q("xqerl_operators:equal(_@E1,_@E2)");
            Op =:= 'ne' ->
                ?Q("xqerl_operators:not_equal(_@E1,_@E2)");
            Op =:= 'gt' ->
                ?Q("xqerl_operators:greater_than(_@E1,_@E2)");
            Op =:= 'ge' ->
                ?Q("xqerl_operators:greater_than_eq(_@E1,_@E2)");
            Op =:= 'lt' ->
                ?Q("xqerl_operators:less_than(_@E1,_@E2)");
            Op =:= 'le' ->
                ?Q("xqerl_operators:less_than_eq(_@E1,_@E2)");
            Op =:= '<<' ->
                ?Q("xqerl_operators:node_before(_@E1,_@E2)");
            Op =:= '>>' ->
                ?Q("xqerl_operators:node_after(_@E1,_@E2)");
            Op =:= 'is' ->
                ?Q("xqerl_operators:node_is(_@E1,_@E2)")
        end,
    Error = var(Line, next_var_name()),
    Result = var(Line, next_var_name()),
    ?P(Line, [
...
expr_do(Ctx, #xqUnaryExpr{anno = Line, sign = Sign, expr = Expr}) ->
    ...
    case Sign of
        '+' ->
            ?P(Line, [
                "case catch xqerl_operators:unary_plus(_@E1) of",
                " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
                " _@Result -> _@Result",
                "end"
            ]);
        '-' ->
            ?P(Line, [
                "case catch xqerl_operators:unary_minus(_@E1) of",
                " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
                " _@Result -> _@Result",
                "end"
            ])
    end;
expr_do(Ctx, #xqArithExpr{
    op = Op,
    lhs = Expr1,
    rhs = Expr2,
    anno = Line
...
expr_do(Ctx, #xqFlwor{id = RetId, loop = Loop, return = Return}) ->
    % Clear any grouping variables
    Ctx1 = Ctx#{grp_variables => []},
...
expr_do(Ctx, #xqTypeswitch{
    ...
    ExpandF = fun(#xqTypeswitchCase{types = Types} = E) ->
    ...
    CaseF = fun
        ...
        (
            ...
            ?Q([
                "case catch xqerl_types:instance_of(_@RootVar,_@AType) of",
                " true -> _@R1;",
                " _ -> _@Acc end"
            ]);
        (
...
expr_do(
    ...
    {double,
        ?CALL(
            #xqFunctionDef{
                params = [A],
                body = {xqerl_mod_fn, Name, 2}
            } = F,
            Line
        )}
) when Name == 'exactly-one'; Name == 'zero-or-one'; Name == 'one-or-more' ->
...
do_axis_step(Ctx, SourceVariable, #xqAxisStep{
    anno = Line,
    id = _Id,
    axis = Axis,
    node_test = NodeTest,
...
handle_axis_step_pred(Ctx, #xqPredicate{
    pos = IsPos,
    anno = Line,
    expr = Pred
}) ->
    ...
    case IsPos of
        false ->
            ?P(Line, [
                "fun(_@IntCtxVar,_@PosVar,_@SizeVar) -> ",
...
abs_path_expr(
    ...
    ok =
        case global_fun_exists(FunNameAtom, 2) of
            ...
            false ->
                Fun = ?Q([
                    "'@FunNameAtom@'(__Ctx,_@CallingVarTup) ->",
                    " _@Expr."
                ]),
                add_global_funs([Fun]),
                ok
    ...
    ?Q([
        "case catch '@FunNameAtom@'(_@CallingCtx,_@CallingVarTup) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]).
...
path_expr_do(Ctx0, #xqPathExpr{anno = Line, expr = [Base | Steps]}) ->
    ...
    case Steps of
        % already doc ordered
        [] when is_record(Base, xqAxisStep) ->
            ?Q(["_@CtxItems"]);
        [] when Line == undefined ->
            ?Q([
                "case xqldb_xpath:document_order(begin  _@CtxItems end) of",
        ...
        _ ->
            % steps will order it
            CtxSeq = ?P(Line, ["InitCtxItem = begin _@CtxItems end"]),
            [CtxSeq | compile_path_statement(Ctx, CtxVar, Steps)]
    end.

get_variable_ref(
    #xqQName{anno = Line, namespace = Ns, prefix = Px, local_name = Ln},
    #{
        tab := Tab,
        parameters := Vars1,
        variables := Vars2,
...
abs_document_node(Ctx, #xqDocumentNode{
    anno = Line,
    content = Contents
}) ->
...
abs_comment_node(Ctx, #xqCommentNode{
    anno = Line,
    type = _,
    text = Text
}) ->
...
abs_pi_node(Ctx, #xqProcessingInstructionNode{
    anno = Line,
    type = direct,
...
abs_namespace_node(_Ctx, #xqNamespaceNode{
    anno = Line,
    type = direct,
    uri = U,
    prefix = P
...
abs_namespace_node(Ctx, #xqNamespaceNode{
    anno = Line,
    type = comp,
    uri = U,
    prefix = P
...
abs_fun_test(Ctx, #xqFunTest{
    ...
    ?Q([
        "#funTest{kind = '@Kind@', annotations = _@E1, name = _@E2,",
...
abs_ns_list(Ctx) ->
    ...
    lists:foldr(
        fun
            (#xqNamespace{prefix = P, namespace = N}, Abs) ->
...
handle_predicate({Ctx, #xqLookup{anno = Line, key = wildcard}}, Abs) ->
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_operators:lookup(_@CtxVar,_@Abs,all) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
handle_predicate({Ctx, #xqLookup{anno = Line, key = Val}}, Abs) ->
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_operators:lookup(_@CtxVar,_@Abs,_@E1) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
...
handle_predicate(
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_seq3:position_filter(_@CtxVar,_@Name1,_@Abs) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
handle_predicate(
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_seq3:position_filter(_@CtxVar,_@A1,_@Abs) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
...
handle_predicate(
    {_Ctx, #xqPredicate{
        pos = true,
        expr = ?CALL(#xqFunctionDef{body = {xqerl_mod_fn, last, _}}, _)
    }},
    Abs
) ->
...
handle_predicate(
    ...
    Result = var(Line, next_var_name()),
    ?P(Line, [
        "case catch xqerl_seq3:position_filter(_@CtxVar,fun(_@NextCtxVar1) ->",
        " _@E1 end,_@Abs) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
...
handle_predicate(
    {Ctx, #xqPredicate{
        anno = Line,
        pos = false,
        expr = #xqAtomicValue{type = Type} = A
    }},
    Abs
...
handle_predicate(
    ...
    E1 = expr_do(Ctx2, P),
    ?P(Line, [
        "case catch xqerl_seq3:filter(_@CtxVar,fun(_@NextCtxVar1,_@IntCtxVar,_@PosVar,_@SizeVar) -> _@E1 end,_@Abs) of",
        " #xqError{} = _@Error -> local_error(_@Error, _@Line@);",
        " _@Result -> _@Result",
        "end"
    ]);
...
% imported var
a_var({_, Name}, CtxVar) when is_atom(Name) ->
    V = atom(?LINE, Name),
...
xpath_function('ancestor', #xqKindTest{kind = 'document-node'}) ->
    {name_type, ancestor_document_node};
xpath_function('ancestor', #xqKindTest{kind = 'element'}) ->
    {name_type, ancestor_element};
xpath_function('ancestor', #xqKindTest{kind = 'node'}) ->
    {none, ancestor_node};
xpath_function('ancestor-or-self', #xqKindTest{kind = 'attribute'}) ->
    {name_type, ancestor_or_self_attribute};
xpath_function('ancestor-or-self', #xqKindTest{kind = 'comment'}) ->
    {none, ancestor_or_self_comment};
xpath_function('ancestor-or-self', #xqKindTest{kind = 'document-node'}) ->
    {name_type, ancestor_or_self_document_node};
xpath_function('ancestor-or-self', #xqKindTest{kind = 'element'}) ->
    {name_type, ancestor_or_self_element};
xpath_function('ancestor-or-self', #xqKindTest{kind = 'node'}) ->
    {none, ancestor_or_self_node};
xpath_function('ancestor-or-self', #xqKindTest{kind = 'processing-instruction'}) ->
    {name, ancestor_or_self_processing_instruction};
xpath_function('ancestor-or-self', #xqKindTest{kind = 'text'}) ->
    {none, ancestor_or_self_text};
xpath_function('attribute', #xqKindTest{kind = 'attribute'}) ->
    {name_type, attribute_attribute};
xpath_function('attribute', #xqKindTest{kind = 'node'}) ->
    {none, attribute_node};
xpath_function('child', #xqKindTest{kind = 'comment'}) ->
    {none, child_comment};
xpath_function('child', #xqKindTest{kind = 'element'}) ->
    {name_type, child_element};
xpath_function('child', #xqKindTest{kind = 'node'}) ->
    {none, child_node};
xpath_function('child', #xqKindTest{kind = 'processing-instruction'}) ->
    {name, child_processing_instruction};
xpath_function('child', #xqKindTest{kind = 'text'}) ->
    {none, child_text};
xpath_function('descendant', #xqKindTest{kind = 'comment'}) ->
    {none, descendant_comment};
xpath_function('descendant', #xqKindTest{kind = 'element'}) ->
    {name_type, descendant_element};
xpath_function('descendant', #xqKindTest{kind = 'node'}) ->
    {none, descendant_node};
xpath_function('descendant', #xqKindTest{kind = 'processing-instruction'}) ->
    {name, descendant_processing_instruction};
xpath_function('descendant', #xqKindTest{kind = 'text'}) ->
    {none, descendant_text};
xpath_function('descendant-or-self', #xqKindTest{kind = 'attribute'}) ->
    {name_type, descendant_or_self_attribute};
xpath_function('descendant-or-self', #xqKindTest{kind = 'comment'}) ->
    {none, descendant_or_self_comment};
xpath_function('descendant-or-self', #xqKindTest{kind = 'document-node'}) ->
    {name_type, descendant_or_self_document_node};
xpath_function('descendant-or-self', #xqKindTest{kind = 'element'}) ->
    {name_type, descendant_or_self_element};
xpath_function('descendant-or-self', #xqKindTest{kind = 'node'}) ->
    {none, descendant_or_self_node};
xpath_function('descendant-or-self', #xqKindTest{kind = 'processing-instruction'}) ->
    {name, descendant_or_self_processing_instruction};
xpath_function('descendant-or-self', #xqKindTest{kind = 'text'}) ->
    {none, descendant_or_self_text};
xpath_function('following', #xqKindTest{kind = 'comment'}) ->
    {none, following_comment};
xpath_function('following', #xqKindTest{kind = 'element'}) ->
    {name_type, following_element};
xpath_function('following', #xqKindTest{kind = 'node'}) ->
    {none, following_node};
xpath_function('following', #xqKindTest{kind = 'processing-instruction'}) ->
    {name, following_processing_instruction};
xpath_function('following', #xqKindTest{kind = 'text'}) ->
    {none, following_text};
xpath_function('following-sibling', #xqKindTest{kind = 'comment'}) ->
    {none, following_sibling_comment};
xpath_function('following-sibling', #xqKindTest{kind = 'element'}) ->
    {name_type, following_sibling_element};
xpath_function('following-sibling', #xqKindTest{kind = 'node'}) ->
    {none, following_sibling_node};
xpath_function('following-sibling', #xqKindTest{kind = 'processing-instruction'}) ->
    {name, following_sibling_processing_instruction};
xpath_function('following-sibling', #xqKindTest{kind = 'text'}) ->
    {none, following_sibling_text};
xpath_function('parent', #xqKindTest{kind = 'document-node'}) ->
    {name_type, parent_document_node};
xpath_function('parent', #xqKindTest{kind = 'element'}) ->
    {name_type, parent_element};
xpath_function('parent', #xqKindTest{kind = 'node'}) ->
    {none, parent_node};
xpath_function('preceding', #xqKindTest{kind = 'comment'}) ->
    {none, preceding_comment};
xpath_function('preceding', #xqKindTest{kind = 'element'}) ->
    {name_type, preceding_element};
xpath_function('preceding', #xqKindTest{kind = 'node'}) ->
    {none, preceding_node};
xpath_function('preceding', #xqKindTest{kind = 'processing-instruction'}) ->
    {name, preceding_processing_instruction};
xpath_function('preceding', #xqKindTest{kind = 'text'}) ->
    {none, preceding_text};
xpath_function('preceding-sibling', #xqKindTest{kind = 'comment'}) ->
    {none, preceding_sibling_comment};
xpath_function('preceding-sibling', #xqKindTest{kind = 'element'}) ->
    {name_type, preceding_sibling_element};
xpath_function('preceding-sibling', #xqKindTest{kind = 'node'}) ->
    {none, preceding_sibling_node};
xpath_function('preceding-sibling', #xqKindTest{kind = 'processing-instruction'}) ->
    {name, preceding_sibling_processing_instruction};
xpath_function('preceding-sibling', #xqKindTest{kind = 'text'}) ->
    {none, preceding_sibling_text};
xpath_function('self', #xqKindTest{kind = 'attribute'}) ->
    {name_type, self_attribute};
xpath_function('self', #xqKindTest{kind = namespace}) ->
    {none, self_namespace};
xpath_function('self', #xqKindTest{kind = 'comment'}) ->
    {none, self_comment};
xpath_function('self', #xqKindTest{kind = 'document-node'}) ->
    {name_type, self_document_node};
xpath_function('self', #xqKindTest{kind = 'element'}) ->
    {name_type, self_element};
xpath_function('self', #xqKindTest{kind = 'node'}) ->
    {none, self_node};
xpath_function('self', #xqKindTest{kind = 'processing-instruction'}) ->
    {name, self_processing_instruction};
xpath_function('self', #xqKindTest{kind = 'text'}) ->
    {none, self_text};
xpath_function(_, _) ->
    {error, empty_axis}.
...
is_simple_step(atomize) ->
    true;
is_simple_step(double) ->
    true;
is_simple_step(#xqAxisStep{
    axis = Axis,
...
maybe_split_predicate_step(
    Sim,
    [
        #xqAxisStep{
            axis = Axis,
            predicates = [#xqPredicate{pos = Pos}] = Preds
        } = H
        | Hs
    ]
) when
...
add_grouping_variable(
    {#qname{} = Qn, _, _, _} = Variable,
    #{
        tab := Tab,
        variables := Vars,
        grp_variables := GVars
...
scan_variable_usage(Loop, Return) ->
    FMap = fun
        (#xqFor{var = #xqVar{id = Id, position = #xqPosVar{id = PId}}}) ->
...
% (for|let)/return
flwor(Ctx, [F], RetId, Return, Internal, Global, TupleVar, Inline, [Rem | Rems]) when
    is_record(F, xqFor); is_record(F, xqLet)
->
...
flwor(
    ...
    is_record(F, xqLet) andalso is_record(N, xqLet);
...
% for/other
flwor(Ctx, [F | T], RetId, Return, Internal, Global, TupleVar, Inline, [Rem | Rems]) when
    is_record(F, xqFor); is_record(F, xqLet)
->
    Vars = flwor_variable_tuple(Ctx),
    NextTupleVar = flwor_next_tuple_variable(TupleVar, Inline),
...
flwor(
    Ctx,
    [#xqWindow{win_variable = #xqVar{id = Id}} = F | T],
    RetId,
    Return,
    Internal,
    Global,
    ...
    flwor(NewCtx1, T, RetId, Return, NewInternal, FunAbs ++ Global, NextTupleVar, false, Rems);
% where
flwor(Ctx, [#xqWhere{id = Id} = W | T], RetId, Return, Internal, Global, TupleVar, _Inline, [
    Rem
    | Rems
]) ->
...
flwor(Ctx, [#xqCount{} = C | T], RetId, Return, Internal, Global, TupleVar, _Inline, [_Rem | Rems]) ->
    ...
    flwor(NewCtx1, T, RetId, Return, NewInternal, FunAbs ++ Global, NextTupleVar, false, Rems);
% group
flwor(Ctx, [#xqGroup{id = Id} = F | T], RetId, Return, Internal, Global, TupleVar, _Inline, [
    Rem
    | Rems
]) ->
    NextTupleVar = var(?LINE, next_var_tuple_name()),
...
flwor(
    Ctx,
    [#xqOrderByClause{anno = L, id = Id} = F | T],
    RetId,
    Return,
    Internal,
    Global,
...
group_part(
    #{
        grp_variables := GrpVars,
        variables := Vars
    } = Ctx,
    #xqGroup{anno = Line, id = Id, vars = Clauses},
    Rem
) ->
    ...
    GrpFun2 =
        ?P(Line, [
            "'@FunctionName@'(_Ctx,_@OutgoingVarTup0,_@CollNT) -> _@ToGroupTuple;",
    ...
    WinCall =
        if
            ...
            true ->
                ...
                ?P(Line, [
                    "case catch xqerl_flwor:windowclause(List,_@StartFunAbs,",
...
for_loop(
    ...
    NewVar = {Name, Type, [], VarName},
    NoEmptyType =
        (Type#xqSeqType.occur == one orelse
...
local_error_fun() ->
    File = get_filename(),
    ?Q([
        "local_error(#xqError{location = undefined} = E, L) ->",
        "    F = _@File@,",
