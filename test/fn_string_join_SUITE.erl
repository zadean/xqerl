...
'fn-string-join2args-1'(Config) ->
   ...
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join2args-1.xq"), Qry1),
...
'fn-string-join2args-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join2args-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "This is a characte") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join2args-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join2args-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "This is a characte") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join2args-4'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join2args-4.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "This is a characte") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join2args-5'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join2args-5.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "This is a characte") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-4'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-4.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "1") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-5'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-5.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "1") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-6'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-6.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "1") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-7'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-7.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "1") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-8'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-8.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-9'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-9.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-10'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-10.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "1") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-11'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-11.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-12'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-12.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-13'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-13.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-14'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-14.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "A") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-15'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-15.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "A") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-16'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-16.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "a") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-17'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-17.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-18'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-18.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-19'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-19.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-20'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-20.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-21'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-21.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "string-join") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-22'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-22.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-23'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-23.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "123") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-24'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-24.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-25'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-25.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-26'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-26.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-28'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-28.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "12345") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-string-join-29'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-29.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-30'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-30.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-31'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-31.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-string-join-32'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "fn-string-join-32.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K-StringJoinFunc-1a'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "K-StringJoinFunc-1a.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K-StringJoinFunc-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "K-StringJoinFunc-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K-StringJoinFunc-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "K-StringJoinFunc-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_true(Res) of 
      true -> {comment, "True"};
      {false, F} -> F 
...
'K-StringJoinFunc-4'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "K-StringJoinFunc-4.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_true(Res) of 
      true -> {comment, "True"};
      {false, F} -> F 
...
'K-StringJoinFunc-5'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "K-StringJoinFunc-5.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_true(Res) of 
      true -> {comment, "True"};
      {false, F} -> F 
...
'K-StringJoinFunc-6'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "K-StringJoinFunc-6.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_true(Res) of 
      true -> {comment, "True"};
      {false, F} -> F 
...
'K-StringJoinFunc-7'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "K-StringJoinFunc-7.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'cbcl-fn-string-join-001'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(BaseDir, "cbcl-fn-string-join-001.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_true(Res) of 
      true -> {comment, "True"};
      {false, F} -> F 
