...
'serialize-xml-008'(_Config) ->
   Qry = "
        ...
        ",
   Env = xqerl_test:handle_environment(environment('ser008')),
...
'serialize-xml-032'(_Config) ->
   Qry = "
          ...
          return serialize(., $params)
        ",
   Env = xqerl_test:handle_environment(environment('ser032')),
   Qry1 = lists:flatten(Env ++ Qry),
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try xqerl:run(Qry1) of D -> D catch _:E -> E end,
...
'serialize-xml-110'(_Config) ->
   Qry = "let $params := map {
            ...
            return serialize(., $params)
        ",
   Env = xqerl_test:handle_environment(environment('ser110')),
   Qry1 = lists:flatten(Env ++ Qry),
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try xqerl:run(Qry1) of D -> D catch _:E -> E end,
   Out =    case lists:all(fun({comment,_}) -> true; (_) -> false end, [
...
{sources, [{"file:///C:/git/zadean/xquery-3.1/QT3-test-suite/fn/serialize/doc001.xml","","http://www.w3.org/fots/serialize-json/doc001.xml"}]},
{schemas, []},
{collections, []},
{'static-base-uri', []},
