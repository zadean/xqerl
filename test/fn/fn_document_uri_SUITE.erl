...
'fn-document-uri-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-document-uri-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-document-uri-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-document-uri-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-document-uri-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-document-uri-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-document-uri-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-document-uri-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-document-uri-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-document-uri-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-document-uri-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-document-uri-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-document-uri-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-document-uri-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-document-uri-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-document-uri-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-document-uri-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-document-uri-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-document-uri-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-document-uri-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-12.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-13.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-14.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-15.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-16.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-17.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-18.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-19.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-20.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "tru") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-document-uri-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-21.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "rue") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-document-uri-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-22.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-document-uri-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-23.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-document-uri-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-24.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-document-uri-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-25.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-document-uri-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-26.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-document-uri-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-27.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-document-uri-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-28.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-29.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-30.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-31'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-31.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-32'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-32.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-33'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-33.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-34'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-34.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-35'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-35.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-36'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-36.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-document-uri-37'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-37.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K-DocumentURIFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DocumentURIFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DocumentURIFunc-2a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DocumentURIFunc-2a.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K-DocumentURIFunc-2b'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DocumentURIFunc-2b.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DocumentURIFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DocumentURIFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-DocumentURIFunc-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-DocumentURIFunc-11.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "int xs:anyURI int") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-document-uri-0-ok'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-document-uri-0-ok.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
