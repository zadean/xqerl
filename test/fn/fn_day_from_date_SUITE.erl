    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date1args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date1args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date1args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "17") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-day-from-date1args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date1args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-day-from-date-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-day-from-date-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DayFromDateFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DayFromDateFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DayFromDateFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DayFromDateFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DayFromDateFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DayFromDateFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DayFromDateFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DayFromDateFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DayFromDateFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DayFromDateFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
