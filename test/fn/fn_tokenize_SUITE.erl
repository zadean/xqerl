...
'fn-tokenize-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "The cat sat on the mat") of
...
'fn-tokenize-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1 15 24 50") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"Some unparsed\", \"HTML <BR> text\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "#r#c#d#r#") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"\", \"bracadabra\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"abracadabra\", \"abracadabra\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-26.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-27.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-28.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"abracadabr\", \"bracadabra\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-29.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"abracadabr\", \"bracadabr\", \"bracadabra\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-30.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-31'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-31.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"abc\", \"def\", \"gh\", \"ijk\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-32'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-32.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"\", \"BR\", \"CAD\", \"BRA\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-33'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-33.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"\", \"RACAD\", \"RA\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-34'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-34.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-35'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-35.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"Mar\", \"ones\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-36'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-36.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-37'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-37.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-38'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-38.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-39'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-39.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-40'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-40.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-41'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-41.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-42'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-42.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-43'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-43.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-44'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-44.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-45'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-45.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-46'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-46.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"abc\", \"def\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-47'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-47.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-48'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-48.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-49'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-49.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_eq(Res, "\"the|quick|brown|fox|jumped|over|the|lazy|dog\"") of
            true -> {comment, "Equal"};
            {false, F} -> F
        end,
    case Out of
...
'fn-tokenize-51'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-51.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-tokenize-52'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-tokenize-52.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TokenizeFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TokenizeFunc-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TokenizeFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TokenizeFunc-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TokenizeFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TokenizeFunc-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TokenizeFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TokenizeFunc-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-TokenizeFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-TokenizeFunc-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-TokenizeFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-TokenizeFunc-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-TokenizeFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-TokenizeFunc-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "r") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-TokenizeFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-TokenizeFunc-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-TokenizeFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-TokenizeFunc-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "true 6 8 The cat sat on the mat") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-TokenizeFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-TokenizeFunc-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "terms") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-TokenizeFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-TokenizeFunc-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2 2 a|b a|b a b a b") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-fn-tokenize-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-fn-tokenize-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-fn-tokenize-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-fn-tokenize-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-fn-tokenize-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
