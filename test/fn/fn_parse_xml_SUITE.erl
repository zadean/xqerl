...
'parse-xml-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-001.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_type(Res, "document-node(element(*,xs:untyped))") of
...
'parse-xml-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-002.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-003.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-004.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-005.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>foo</a>") of
...
'parse-xml-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-006.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-007.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-008.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-009.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-010.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a><bar>baz</bar></a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'parse-xml-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-011.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-012.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-013.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-014.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-015.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-016.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'parse-xml-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "parse-xml-017.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
