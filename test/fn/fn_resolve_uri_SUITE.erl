...
'fn-resolve-uri-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example/") of
...
'fn-resolve-uri-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.examples.com/examples") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-resolve-uri-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.com/examples") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-resolve-uri-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.com/a.html") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-resolve-uri-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-26.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-27.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.com/b.html") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-resolve-uri-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-29.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "urn:isbn:01234567890X") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-resolve-uri-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-30.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-31'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-31.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-resolve-uri-32'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-resolve-uri-32.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case
            lists:any(
                ...
                [
                    case
                        xqerl_test:assert_string_value(Res, "http://www.example.com/this doc.html")
                    of
                        true -> {comment, "String correct"};
                        {false, F} -> F
                    end,
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ResolveURIFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ResolveURIFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ResolveURIFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ResolveURIFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ResolveURIFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ResolveURIFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ResolveURIFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ResolveURIFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ResolveURIFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ResolveURIFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ResolveURIFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-fn-resolve-uri-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-fn-resolve-uri-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
