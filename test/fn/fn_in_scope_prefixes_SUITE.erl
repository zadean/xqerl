...
environment('NamespaceSuppliedInternally', __BaseDir) ->
    [
        {'decimal-formats', []},
        {sources, [
            {filename:join(__BaseDir, "in-scope-prefixes/NamespaceSuppliedInternally.xml"), ".", []}
        ]},
        {collections, []},
        {'static-base-uri', []},
        {params, []},
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2 true") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-in-scope-prefixes-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "120 109 108") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-in-scope-prefixes-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "xmlm") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-in-scope-prefixes-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "xmlxml") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-in-scope-prefixes-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "ml") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-in-scope-prefixes-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-in-scope-prefixes-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-21.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-in-scope-prefixes-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-22.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-in-scope-prefixes-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-23.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-in-scope-prefixes-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-25.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"\", \"xml\", \"xlink\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-in-scope-prefixes-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-in-scope-prefixes-26.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K-InScopePrefixesFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-InScopePrefixesFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-InScopePrefixesFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-InScopePrefixesFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "a p xml") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-InScopePrefixesFunc-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "2, 1, 2, 2") of
...
'K2-InScopePrefixesFunc-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-22.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-23.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-24.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-25.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "a b xml | a xml") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-InScopePrefixesFunc-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-26.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-27.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "a b xml") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-InScopePrefixesFunc-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-28.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-InScopePrefixesFunc-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-29.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case
            xqerl_test:assert_xml(Res, "<e xmlns=\"http://www.example.com/\"><a xmlns=\"\"/></e>")
        of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
...
'K2-InScopePrefixesFunc-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-InScopePrefixesFunc-30.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
