...
'path001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path001.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path002.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path003.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path004.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path005.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path006.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path007.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path008.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path009.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path010.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path014.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
...
'path015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path015.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'path016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path016.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'path017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path017.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'path018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path018.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'path019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path019.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'path020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "path020.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
