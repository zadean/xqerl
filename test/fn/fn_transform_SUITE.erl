...
environment('works-mod-uri2', __BaseDir) ->
    [
        ...
        {sources, [
            ...
            {filename:join(__BaseDir, "transform/render.xsl"), "",
                "http://www.w3.org/fots/fn/transform/render.xsl"}
        ]},
        {collections, []},
        {'static-base-uri', []},
        {params, []},
        {vars, [
            {"uri", "xs:string", "'http://www.w3.org/fots/docs/works-mod.xml'"},
...
environment('works-mod-local', __BaseDir) ->
    [
        {'decimal-formats', []},
        {sources, [
            {filename:join(__BaseDir, "../docs/works-mod.xml"), "$works", []},
...
environment('variable-uri', __BaseDir) ->
    [
        {'decimal-formats', []},
        {sources, [
            {filename:join(__BaseDir, "transform/variable.xsl"), ".",
                "http://www.w3.org/fots/fn/transform/variable.xsl"}
        ]},
        {collections, []},
        {'static-base-uri', []},
        {params, []},
        {vars, [
            {"variable-uri", "xs:string", "'http://www.w3.org/fots/fn/transform/variable.xsl'"}
        ]},
        {namespaces, []},
        {schemas, []},
        {resources, []},
...
environment('variable-with-context', __BaseDir) ->
    [
        {'decimal-formats', []},
        {sources, [
            {filename:join(__BaseDir, "transform/variable-with-context.xsl"), "$stylesheet", []}
        ]},
        {collections, []},
        {'static-base-uri', []},
        {params, []},
...
environment('multiple-result-docs', __BaseDir) ->
    [
        {'decimal-formats', []},
        {sources, [
            {filename:join(__BaseDir, "transform/multipledocs.xsl"), "$multipledocs", []},
...
'fn-transform-901'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "let $result := fn:transform(map {\"stylesheet-location\" : $render, \"source-node\" : fn:doc($uri)})return $result(\"output\")",
    {Env, Opts} = xqerl_test:handle_environment(environment('works-mod-uri2', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-transform-901.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-transform-902'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-transform-902.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
