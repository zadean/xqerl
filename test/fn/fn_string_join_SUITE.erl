    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-string-join2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-string-join2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-string-join2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-string-join2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-string-join2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-string-join2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-string-join2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-string-join2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-string-join2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-string-join-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "Now is the time ...") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-string-join-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "Blow, blow, thou winter wind!") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-string-join-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "AAAAABBBBBCCCCC") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-string-join-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "AAAAABBBBB") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-string-join-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "string-join") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-string-join-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "string-joinstring-join") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-string-join-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "13") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-string-join-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-26.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "123456789") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-string-join-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-28.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-29.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-30.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1900,1900,1900,1900,1900") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-string-join-31'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-31.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-string-join-32'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-string-join-32.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "b-c-d") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K-StringJoinFunc-1a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StringJoinFunc-1a.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StringJoinFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StringJoinFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StringJoinFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StringJoinFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StringJoinFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StringJoinFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StringJoinFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StringJoinFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StringJoinFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StringJoinFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StringJoinFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StringJoinFunc-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-fn-string-join-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-fn-string-join-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
