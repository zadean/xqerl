    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time1args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time1args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time1args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time1args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time1args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-time-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-time-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromTimeFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromTimeFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromTimeFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromTimeFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromTimeFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromTimeFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromTimeFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromTimeFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromTimeFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromTimeFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
