    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration1args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration1args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration1args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration1args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration1args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-hours-from-duration-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-hours-from-duration-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromDurationFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromDurationFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromDurationFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromDurationFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromDurationFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromDurationFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromDurationFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromDurationFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromDurationFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromDurationFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromDurationFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromDurationFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-HoursFromDurationFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-HoursFromDurationFunc-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-hours-from-duration-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-hours-from-duration-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
