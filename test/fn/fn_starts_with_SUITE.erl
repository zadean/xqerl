    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-starts-with2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-starts-with2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-starts-with2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-starts-with2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-starts-with2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-starts-with2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-starts-with2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-starts-with2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-starts-with2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-starts-with-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-19'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:starts-with(\"database\", \"DATA\", \"http://www.w3.org/2013/collation/UCA?lang=en;strength=primary\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-20'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:starts-with(\"database\", \"databaS\", \"http://www.w3.org/2013/collation/UCA?lang=en;strength=primary\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-22'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:starts-with(\"dâtabase\", \"dâtabasen\", \"http://www.w3.org/2013/collation/UCA?lang=en;strength=primary\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-24'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:starts-with(\"database\", \"DATA\", \"http://www.w3.org/2013/collation/UCA?lang=en;strength=secondary\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-25'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:starts-with(\"database\", \"databaS\", \"http://www.w3.org/2013/collation/UCA?lang=en;strength=secondary\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-26'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:starts-with(\"database\", \"dâta\", \"http://www.w3.org/2013/collation/UCA?lang=en;strength=secondary\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-26.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-27'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:starts-with(\"dâtabase\", \"Data\", \"http://www.w3.org/2013/collation/UCA?lang=en;strength=secondary\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-27.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-28'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:starts-with(\"database\", \"DATA\", \"http://www.w3.org/2013/collation/UCA?lang=en;strength=tertiary\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-28.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-29'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:starts-with(\"dataBase\", \"dataBas\", \"http://www.w3.org/2013/collation/UCA?lang=en;strength=tertiary\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-29.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-30'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:starts-with(\"database\", \"dâta\", \"http://www.w3.org/2013/collation/UCA?lang=en;strength=tertiary\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-30.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-starts-with-31'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-starts-with-31.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-StartsWithFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StartsWithFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StartsWithFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StartsWithFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StartsWithFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StartsWithFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StartsWithFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StartsWithFunc-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StartsWithFunc-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StartsWithFunc-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StartsWithFunc-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StartsWithFunc-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-StartsWithFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-StartsWithFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-StartsWithFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-StartsWithFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-StartsWithFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-StartsWithFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-StartsWithFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-StartsWithFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-StartsWithFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-StartsWithFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-StartsWithFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-StartsWithFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-starts-with-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-starts-with-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
