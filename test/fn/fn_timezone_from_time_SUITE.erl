    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-time-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-time-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromTimeFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromTimeFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromTimeFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromTimeFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromTimeFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromTimeFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromTimeFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromTimeFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromTimeFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromTimeFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromTimeFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromTimeFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromTimeFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromTimeFunc-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
