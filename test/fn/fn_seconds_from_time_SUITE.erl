    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time1args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time1args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time1args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "35") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-seconds-from-time1args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time1args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "10.5") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-seconds-from-time-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "25") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-seconds-from-time-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "27") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-seconds-from-time-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-seconds-from-time-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-seconds-from-time-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SecondsFromTimeFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SecondsFromTimeFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SecondsFromTimeFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SecondsFromTimeFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SecondsFromTimeFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SecondsFromTimeFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SecondsFromTimeFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SecondsFromTimeFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SecondsFromTimeFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SecondsFromTimeFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
