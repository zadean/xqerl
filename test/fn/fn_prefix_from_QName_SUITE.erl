    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-prefix-from-qname-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-prefix-from-qname-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-prefix-from-qname-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-prefix-from-qname-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-5.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-6.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-prefix-from-qname-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-prefix-from-qname-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-9.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-10.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-11.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "foo:bar") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-prefix-from-qname-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-12.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "f") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-prefix-from-qname-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-13.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "oo") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-prefix-from-qname-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-14.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-15.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-16.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-17.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-18.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-19.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-20.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-prefix-from-qname-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-prefix-from-qname-23.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K-PrefixFromQName-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-PrefixFromQName-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-PrefixFromQName-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-PrefixFromQName-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-PrefixFromQName-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-PrefixFromQName-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-PrefixFromQName-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-PrefixFromQName-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
