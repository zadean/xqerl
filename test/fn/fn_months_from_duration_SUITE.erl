    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration1args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration1args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration1args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration1args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration1args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-months-from-duration-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-months-from-duration-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-MonthsFromDurationFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-MonthsFromDurationFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-MonthsFromDurationFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-MonthsFromDurationFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-MonthsFromDurationFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-MonthsFromDurationFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-MonthsFromDurationFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-MonthsFromDurationFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-MonthsFromDurationFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-MonthsFromDurationFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-MonthsFromDurationFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-MonthsFromDurationFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-MonthsFromDurationFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-MonthsFromDurationFunc-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-months-from-duration-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-months-from-duration-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
