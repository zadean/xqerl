...
'fn-static-base-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-static-base-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "ftp://ftp.is.co.za/rfc/somefile.txt") of
...
'fn-static-base-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "mailto:John.Doe@example.com") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-static-base-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "news:comp.infosystems.www.servers.unix") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-static-base-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "telnet://192.0.2.16:80/") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-static-base-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "tel:+1-816-555-1212") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-static-base-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-static-base-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-static-base-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-static-base-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.comanother string") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-static-base-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-static-base-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "22") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-static-base-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-static-base-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-static-base-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "//www.example.com") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StaticBaseURIFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StaticBaseURIFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StaticBaseURIFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-StaticBaseURIFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-StaticBaseURIFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
