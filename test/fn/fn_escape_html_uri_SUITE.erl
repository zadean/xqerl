    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri1args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri1args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri1args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri1args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri1args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri1args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri1args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri1args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri1args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri1args-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri1args-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "abcdedfghijklmnopqrstuvwxyz") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-escape-html-uri-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "a0123456789") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-escape-html-uri-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-escape-html-uri-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "example%C3%A9%E2%82%ACexample") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-escape-html-uri-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-escape-html-uri-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "example%E2%82%ACexample") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K-EscapeHTMLURIFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-EscapeHTMLURIFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-EscapeHTMLURIFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-EscapeHTMLURIFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-EscapeHTMLURIFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-EscapeHTMLURIFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-EscapeHTMLURIFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-EscapeHTMLURIFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-EscapeHTMLURIFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-EscapeHTMLURIFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-EscapeHTMLURIFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-EscapeHTMLURIFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "example.com") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'cbcl-escape-html-uri-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-escape-html-uri-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "%09A%C2%80") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
