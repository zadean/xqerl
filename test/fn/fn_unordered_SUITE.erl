...
'fn-unordered-mix-args-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"c\", 1, \"xzy\"") of
...
'fn-unordered-mix-args-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"c\",  \"b\", \"a\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unordered-mix-args-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", \"www.example.com\", \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unordered-mix-args-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", 100, -100, \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-008.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", -1.000000000001, -100, \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-009.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", xs:float(\"INF\"), \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-010.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unordered-mix-args-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-011.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", xs:float('NaN'), \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-012.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", 1.01, \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-013.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", xs:double('NaN'), \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unordered-mix-args-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-015.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", xs:double('-INF'), \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-016.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unordered-mix-args-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-017.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", true(), \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-018.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", false(), \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-019.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unordered-mix-args-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-020.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unordered-mix-args-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-021.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", xs:date('1993-03-31'), \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unordered-mix-args-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-022.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unordered-mix-args-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unordered-mix-args-023.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_permutation(Res, "\"a\", xs:time('12:30:00'), \"b\", \"c\"") of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
        end,
    case Out of
...
'K-SeqUnorderedFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SeqUnorderedFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SeqUnorderedFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SeqUnorderedFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SeqUnorderedFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SeqUnorderedFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SeqUnorderedFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SeqUnorderedFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SeqUnorderedFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SeqUnorderedFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SeqUnorderedFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SeqUnorderedFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SeqUnorderedFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SeqUnorderedFunc-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-SeqUnorderedFunc-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-SeqUnorderedFunc-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-fn-unordered-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-fn-unordered-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-fn-unordered-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-fn-unordered-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-fn-unordered-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-fn-unordered-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unordered-101'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-unordered-101.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-unordered-102'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-unordered-102.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-unordered-103'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-unordered-103.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-unordered-104'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-unordered-104.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-unordered-105'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-unordered-105.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-unordered-106'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-unordered-106.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-unordered-107'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-unordered-107.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-unordered-108'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-unordered-108.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case
            xqerl_test:assert_permutation(
                Res,
                "40, 140, 240, 80, 100, 72, 280, 640, 180, 200, 220, 480, 1040"
            )
        of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
...
'fn-unordered-109'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-unordered-109.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case
            xqerl_test:assert_permutation(
                Res,
                "520, 840, 880, 200, 180, 96, 280, 480, 100, 80, 60, 80, 80"
            )
        of
            true -> {comment, "Correct permutation"};
            {false, F} -> F
