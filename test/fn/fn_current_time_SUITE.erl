...
'fn-current-time-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-current-time-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-current-time-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-current-time-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-current-time-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-current-time-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-current-time-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-current-time-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-current-time-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-current-time-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-current-time-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-current-time-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-current-time-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-current-time-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-current-time-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-current-time-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-current-time-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-current-time-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-current-time-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-current-time-22.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ContextCurrentTimeFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ContextCurrentTimeFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ContextCurrentTimeFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ContextCurrentTimeFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
