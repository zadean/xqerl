    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.com/defaultspace") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-namespace-uri-for-prefix-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-22.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-namespace-uri-for-prefix-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-namespace-uri-for-prefix-23.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NamespaceURIForPrefixFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NamespaceURIForPrefixFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NamespaceURIForPrefixFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NamespaceURIForPrefixFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceURIForPrefixFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceURIForPrefixFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.com/B") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-NamespaceURIForPrefixFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceURIForPrefixFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceURIForPrefixFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceURIForPrefixFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.com/A") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'cbcl-namespace-uri-for-prefix-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-namespace-uri-for-prefix-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
