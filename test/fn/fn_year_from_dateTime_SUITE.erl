...
'fn-year-from-dateTime1args-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime1args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1970") of
...
'fn-year-from-dateTime1args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime1args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1996") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-year-from-dateTime1args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime1args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2030") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-year-from-dateTime-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1998") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-year-from-dateTime-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "3940") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-year-from-dateTime-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTimeNew-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTimeNew-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1971") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-year-from-dateTimeNew-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTimeNew-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-dateTime-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-dateTime-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearFromDateTimeFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearFromDateTimeFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearFromDateTimeFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearFromDateTimeFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearFromDateTimeFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearFromDateTimeFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearFromDateTimeFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearFromDateTimeFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearFromDateTimeFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearFromDateTimeFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
