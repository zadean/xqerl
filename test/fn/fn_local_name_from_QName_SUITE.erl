    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "LocalNameFromQNameFunc006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'LocalNameFromQNameFunc007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "LocalNameFromQNameFunc007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'LocalNameFromQNameFunc009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "LocalNameFromQNameFunc009.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'LocalNameFromQNameFunc011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "LocalNameFromQNameFunc011.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'LocalNameFromQNameFunc015a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "LocalNameFromQNameFunc015a.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'LocalNameFromQNameFunc016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "LocalNameFromQNameFunc016.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'LocalNameFromQNameFunc017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "LocalNameFromQNameFunc017.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-local-name-from-qname-024'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-local-name-from-qname-024.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K-LocalNameFromQNameFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-LocalNameFromQNameFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-LocalNameFromQNameFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-LocalNameFromQNameFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-LocalNameFromQNameFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-LocalNameFromQNameFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-LocalNameFromQNameFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-LocalNameFromQNameFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
