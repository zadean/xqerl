    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-008.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-009.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-011.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-013.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-015.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-016.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "99") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-function-arity-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-017.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-018.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-019.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-020.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-021.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-function-arity-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-022.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-function-arity-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-function-arity-023.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
