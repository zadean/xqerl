...
environment('default-collection-1', __BaseDir) ->
    [
        ...
        {collections, [
            {"", [
                {src, filename:join(__BaseDir, "../docs/bib.xml")},
...
environment('simple-collection-1', __BaseDir) ->
    [
        ...
        {sources, []},
        {collections, [
            {"http://www.w3.org/2010/09/qt-fots-catalog/collection1", [
                {src, filename:join(__BaseDir, "../docs/bib.xml")},
                {src, filename:join(__BaseDir, "../docs/reviews.xml")}
            ]}
        ]},
...
environment('simple-collection-2', __BaseDir) ->
    [
        ...
        {sources, []},
        {collections, [
            {"http://www.w3.org/2010/09/qt-fots-catalog/collection2", [
                {src, filename:join(__BaseDir, "../docs/bib.xml")},
                {src, filename:join(__BaseDir, "../docs/reviews.xml")},
                {src, filename:join(__BaseDir, "../docs/books.xml")}
            ]}
...
environment('integer-collection', __BaseDir) ->
    [
        ...
        {sources, []},
        {collections, [
            {"http://www.w3.org/2010/09/qt-fots-catalog/integers", [{query, __BaseDir, "1 to 10"}]}
        ]},
        {'static-base-uri', []},
        {params, []},
        {vars, []},
...
environment('atomic-collection', __BaseDir) ->
    [
        ...
        {sources, []},
        {collections, [
            {"http://www.w3.org/2010/09/qt-fots-catalog/atomics", [
                {query, __BaseDir, "(1, \"hello\", 1e0)"}
            ]}
        ]},
        {'static-base-uri', []},
        {params, []},
...
'fn-collection-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-collection-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-collection-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-collection-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-4.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-collection-4d'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-4d.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-collection-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-5.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-collection-5d'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-5d.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-collection-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-6.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-collection-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-7.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-collection-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-8.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-collection-9'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "for $d in fn:collection($collection-uri) order by count($d//title) return\n"
        "         count($d//title)",
    {Env, Opts} = xqerl_test:handle_environment(environment('simple-collection-2', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-9.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-collection-10'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        " let $c1 := fn:collection($collection-uri) let $c2 := fn:collection($collection-uri) for\n"
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-10.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-collection-10d'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        " let $c1 := fn:collection() let $c2 := fn:collection() for $c at $p in $c1 return $c is\n"
        "         exactly-one($c2[$p])",
    {Env, Opts} = xqerl_test:handle_environment(environment('default-collection-2', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-collection-10d.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqCollectionFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqCollectionFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqCollectionFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'collection-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-001.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'collection-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-002.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'collection-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-003.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'collection-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-005.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'collection-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-006.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'collection-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-007.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'collection-009'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "let $c := collection(\"http://www.w3.org/2010/09/qt-fots-catalog/collection1\") return $c\n"
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-009.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'collection-900'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-900.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'collection-901'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-901.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'collection-902'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-902.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'collection-903'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "collection-903.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-collection-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-collection-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-collection-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-collection-002.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'cbcl-collection-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-collection-003.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'cbcl-collection-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-collection-004.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
