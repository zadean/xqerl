...
environment('nillable', __BaseDir) ->
    [
        ...
        {schemas, [
            {filename:join(__BaseDir, "nilled/nillable.xsd"),
...
environment('validate', __BaseDir) ->
    [
        ...
        {namespaces, []},
        {schemas, [
            {filename:join(__BaseDir, "nilled/validate.xsd"),
...
'fn-nilled-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-2.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-nilled-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-3.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-nilled-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'fn-nilled-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-23.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-nilled-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-24.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-nilled-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-25.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-nilled-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-26.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-nilled-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-27.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-nilled-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-28.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-nilled-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-29.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'fn-nilled-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "fn-nilled-30.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'K-NilledFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-NilledFunc-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-NilledFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-NilledFunc-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-NilledFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-NilledFunc-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-nilled-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-nilled-007.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
