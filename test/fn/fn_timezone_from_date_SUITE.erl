    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "PT11H") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-timezone-from-date-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-timezone-from-date-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-timezone-from-date-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromDateFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromDateFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromDateFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromDateFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromDateFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromDateFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromDateFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromDateFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromDateFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromDateFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromDateFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromDateFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimezoneFromDateFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-TimezoneFromDateFunc-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-timezone-from-date-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-timezone-from-date-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-timezone-from-date-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-timezone-from-date-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-timezone-from-date-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-timezone-from-date-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "PT14H") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'cbcl-timezone-from-date-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-timezone-from-date-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-timezone-from-date-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-timezone-from-date-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-timezone-from-date-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-timezone-from-date-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-timezone-from-date-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-timezone-from-date-007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
