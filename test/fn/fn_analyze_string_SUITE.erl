...
environment('analyze-string-schema', __BaseDir) ->
    [
        ...
        {namespaces, []},
        {schemas, [
            {filename:join(__BaseDir, "analyze-string/analyze-string.xsd"),
...
'analyzeString-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-001.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-002.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "analyzeString-002a.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'analyzeString-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-003.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-004.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-005.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-006.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-007.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-008.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-009.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-010.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-011.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-012.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-013.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-014.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-015.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-016.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-017.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-017a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "analyzeString-017a.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'analyzeString-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-018.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-019.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-022.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "banana") of
...
'analyzeString-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-023.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-026'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-026.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-027'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-027.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_eq(Res, "concat('y', codepoints-to-string(13), 'J')") of
            true -> {comment, "Equal"};
            {false, F} -> F
        end,
    case Out of
...
'analyzeString-028'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-028.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case
            xqerl_test:assert_deep_eq(
                Res,
                "\"http://www.w3.org/2005/xpath-functions\", \"http://www.w3.org/XML/1998/namespace\""
            )
        of
...
'analyzeString-029'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-029.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
            ...
            lists:all(
                ...
                [
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[1]/fn:match[1]/fn:group[@nr=1] = '/OPDH/'"
                        )
                    of
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[1]/fn:match[1]/fn:group[@nr=2] = 'FLOWING SOLUTION'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[1]/fn:match[1]/fn:group[@nr=3] = '/SGDE/'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[1]/fn:match[1]/fn:group[@nr=4] = 'Sample test/some other keys/'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[1]/fn:match[1]//fn:group[@nr=5][../@nr=4] = 'Sample test'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[1]/fn:match[1]//fn:group[@nr=6][../@nr=4] = '/some other keys/'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[2]/fn:match[1]/fn:group[@nr=1] = '/some other keys/'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[2]/fn:match[1]/fn:group[@nr=2] = 'afdsf'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[2]/fn:match[1]/fn:group[@nr=3] = '/SGDE/'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[2]/fn:match[1]/fn:group[@nr=4] = 'Sample test/some other keys/'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[2]/fn:match[1]//fn:group[@nr=5][../@nr=4] = 'Sample test'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/out/fn:analyze-string-result[2]/fn:match[1]//fn:group[@nr=6][../@nr=4] = '/some other keys/'"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
...
'analyzeString-901'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-901.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-902'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-902.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'analyzeString-903'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "analyzeString-903.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
