    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date1args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date1args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date1args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1983") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-year-from-date1args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date1args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-year-from-date-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-year-from-date-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearFromDateFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearFromDateFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearFromDateFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearFromDateFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearFromDateFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearFromDateFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearFromDateFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearFromDateFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearFromDateFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearFromDateFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
