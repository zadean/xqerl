...
environment('unparsed-text', __BaseDir) ->
    [
        ...
        {schemas, []},
        {resources, [
            {"text/plain", filename:join(__BaseDir, "unparsed-text/text-with-surrogates.txt"),
                "http://www.w3.org/fots/unparsed-text/text-with-surrogates.txt"},
            {"text/plain",
            ...
            {"text/plain",
                filename:join(__BaseDir, "unparsed-text/text-plain-utf-8-bom-invalid.txt"),
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-008.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-010.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-012.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-013.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-015.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-016.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-017.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-018.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-019.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-020.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-021.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-022.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-unparsed-text-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-023.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-024'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-024.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-025'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-025.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-026'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-026.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-027'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-027.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-028'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-028.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-029'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-029.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-030'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-030.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-031'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-031.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-032'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-032.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-033'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-033.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-034'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-034.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-035'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-035.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-036'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-036.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-037'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-037.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-038'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-038.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-039'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-039.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-040'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-040.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-041'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-041.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-042'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-042.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-043'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-043.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-044'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-044.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-045'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-045.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-046'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-046.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-047'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-047.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-048'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-048.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-049'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-049.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "400") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-unparsed-text-050'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-050.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-051'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-051.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-052'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-052.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-053'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-053.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-unparsed-text-055'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-unparsed-text-055.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_eq(Res, "2048") of
            true -> {comment, "Equal"};
            {false, F} -> F
        end,
    case Out of
