...
'for-each-pair-001'(Config) ->
    ...
    Qry =
        "for-each-pair((\"aa\", \"bb\", \"cc\", \"dd\", \"ee\"), (\"AA\", \"BB\", \"cc\", \"dd\", \"EE\"), deep-equal#2)",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "for-each-pair-001.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "false(), false(), true(), true(), false()") of
...
'for-each-pair-002'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "for-each-pair((\"aa\", \"bb\", \"cc\", \"dd\", \"ee\", \"ff\"), (\"AA\", \"BB\", \"cc\", \"dd\", \"EE\"), deep-equal#2)",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "for-each-pair-002.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'for-each-pair-003'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "for-each-pair((\"aa\", \"bb\", \"cc\", \"dd\", \"ee\"), (\"AA\", \"BB\", \"cc\", \"dd\", \"EE\", \"ff\"), deep-equal#2)",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "for-each-pair-003.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'for-each-pair-004'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "for-each-pair((\"aa\", \"bb\", \"cc\", \"dd\", \"ee\"), (\"AA\", \"BB\", \"cc\", \"dd\", \"EE\"), concat(?, '-', ?))",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "for-each-pair-004.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "'aa-AA', 'bb-BB', 'cc-cc', 'dd-dd', 'ee-EE'") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'for-each-pair-005'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "for-each-pair(1 to 5, 1 to 5, function($a as xs:integer, $b as xs:integer) as xs:integer{$a + $b})",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "for-each-pair-005.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'for-each-pair-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "for-each-pair-006.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "5, 7, 9, 11, 13") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'for-each-pair-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "for-each-pair-007.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "1, 1, 2, 1, 2, 3") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'for-each-pair-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "for-each-pair-008.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "3, 5, 7, 9") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'for-each-pair-901'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "for-each-pair((\"aa\", \"bb\", \"cc\", \"dd\", \"ee\"), (\"AA\", \"BB\", \"cc\", \"dd\", \"EE\"), deep-equal#3)",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "for-each-pair-901.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'for-each-pair-902'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "for-each-pair((\"aa\", \"bb\", \"cc\", \"dd\", \"ee\"), (\"AA\", \"BB\", \"cc\", \"dd\", 12), contains#2)",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "for-each-pair-902.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-008.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-009.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-011.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-012.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-013.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-for-each-pair-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-015.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-016'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:for-each-pair( (), (), function($a as item(), $b as item()) as item()* { fn:boolean($a), fn:boolean($b) } )",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-016.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-017'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:for-each-pair((//node(), 1, \"string\", 3.14, 2.7e0, fn:exists#1),\n"
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-017.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-for-each-pair-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-018.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-019.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-020.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-021.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-022.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-023.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-024'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-024.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-025'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-025.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2 4 6") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-for-each-pair-026'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:for-each-pair( (true(), false(), true()), 1 to 3, function($arg1, $arg2) { if ($arg1) then $arg2 else () })",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-026.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-027'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-027.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1 1 2 2 3 3") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-for-each-pair-028'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "fn:for-each-pair( (\"1\", \"2\"), (1, 2), function($arg1 as xs:integer, $arg2 as xs:integer) { $arg1 + $arg2 } )",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-028.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-029'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-029.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-030'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-030.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-031'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-031.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-033'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-033.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-034'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-034.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-035'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-035.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-for-each-pair-036'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-036.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-for-each-pair-037'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-for-each-pair-037.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
