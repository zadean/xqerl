...
'parse-xml-fragment-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-001.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_type(Res, "element(*,xs:untyped)") of
...
'parse-xml-fragment-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-002.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-003.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-004.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-005.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-006.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "vanessa") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'parse-xml-fragment-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-007.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-008.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-009.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-010.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
        ...
        case
            lists:all(
                ...
                [
                    ...
                    case
                        xqerl_test:assert(Res, "$result/node()[2][self::comment()][. = 'comment']")
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    end,
                    case
                        xqerl_test:assert(
                            Res,
                            "$result/node()[3][self::processing-instruction(PI)][. = '']"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
...
'parse-xml-fragment-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-011.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-012.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-013.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-014.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-015.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-016.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-017.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-018.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-019.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-020.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-021.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "97 10 98") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'parse-xml-fragment-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-022.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-023.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-024'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-024.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'parse-xml-fragment-025'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "parse-xml-fragment-025.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
