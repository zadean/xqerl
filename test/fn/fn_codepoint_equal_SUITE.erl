    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-2a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-2a.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-2b'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-2b.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-codepoint-equal-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-codepoint-equal-22.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-CodepointEqual-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-CodepointEqual-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-CodepointEqual-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-CodepointEqual-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-CodepointEqual-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-CodepointEqual-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-CodepointEqual-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-CodepointEqual-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-CodepointEqual-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-CodepointEqual-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-CodepointEqual-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-CodepointEqual-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-codepoint-equal-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-codepoint-equal-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-codepoint-equal-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-codepoint-equal-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-codepoint-equal-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-codepoint-equal-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-codepoint-equal-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-codepoint-equal-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-codepoint-equal-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-codepoint-equal-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-codepoint-equal-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-codepoint-equal-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
