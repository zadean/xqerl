...
'fn-normalize-space1args-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space1args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "This is a charac") of
...
'fn-normalize-space1args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space1args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "This is a ch") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-normalize-space1args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space1args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space1args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space1args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space0args-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space0args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "Hello, How are you?") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-normalize-space-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "The wealthy curled darlings of our nation.") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-normalize-space-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-2.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-normalize-space-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "This text should contains no tabs") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-normalize-space-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case
            xqerl_test:assert_string_value(Res, "This text should contains no newline characters.")
        of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
...
'fn-normalize-space-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "This text should contains no tab characters.") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-normalize-space-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-normalize-space-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-normalize-space-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NormalizeSpaceFunc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NormalizeSpaceFunc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NormalizeSpaceFunc-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NormalizeSpaceFunc-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NormalizeSpaceFunc-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NormalizeSpaceFunc-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NormalizeSpaceFunc-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NormalizeSpaceFunc-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NormalizeSpaceFunc-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NormalizeSpaceFunc-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NormalizeSpaceFunc-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NormalizeSpaceFunc-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NormalizeSpaceFunc-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NormalizeSpaceFunc-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NormalizeSpaceFunc-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NormalizeSpaceFunc-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NormalizeSpaceFunc-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NormalizeSpaceFunc-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
