...
'map-merge-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-001.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-002.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-003-hof'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-003-hof.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-003.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-004.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-005.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-006'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "map:merge((map:entry(\"foo\", 3), map:entry(\"foo\", 4)), map{\"duplicates\": \"use-last\"})",
    {Env, Opts} = xqerl_test:handle_environment(environment('map', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-006.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-006b'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "map:merge((map:entry(\"foo\", 3), map:entry(\"foo\", 4)), map{\"duplicates\": \"use-first\"})",
    {Env, Opts} = xqerl_test:handle_environment(environment('map', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-006b.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-006c'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "map:merge((map:entry(\"foo\", 3), map:entry(\"foo\", 4)), map{\"duplicates\": \"combine\"})",
    {Env, Opts} = xqerl_test:handle_environment(environment('map', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-006c.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-006d'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-006d.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-006e'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-006e.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-006f'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-006f.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-007'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "let $foo := map:entry(\"foo\", 3), $bar := map:entry(\"foo\", 4), $foobar := map:merge(($foo, $bar))\n"
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-007.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-008'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "let $foo := map:merge(for $i in 1 to 20 return map:entry($i, $i*$i)), $bar := map:entry(8, 63), $foobar := map:merge(($foo, $bar))\n"
        "              return ($foobar, $bar, $foo)[3]",
    {Env, Opts} = xqerl_test:handle_environment(environment('map', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-008.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-009.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-010'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "let $foo := map:merge(for $i in 1 to 20 return map:entry($i, $i*$i)), $bar := map:merge(()), $foobar := map:merge(($foo, $bar))\n"
        "              return $foobar",
    {Env, Opts} = xqerl_test:handle_environment(environment('map', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-010.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-011'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "map:merge((map:entry(3, \"three\"), map:entry(3.0e0, \"threeD\"), map:entry(xs:float('3.0'), \"threeF\")),\n"
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-011.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-012'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "map:merge(for $n in distinct-values(//*/node-name()) \n"
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-012.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-013.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-016.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-017.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-018.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-019.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-020'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "deep-equal(map:merge(for $i in 1 to 1000 return map:entry($i, $i+1)),\n"
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-020.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-021.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-022.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-023.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-024-hof'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-024-hof.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-024'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-024.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'map-merge-025'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-025.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "1 to 2000") of
...
'map-merge-026'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "map:merge((1 to 100)!map:entry('z', .), if (current-date() lt xs:date('1900-01-01')) then map{'duplicates':'combine'} else ())?z",
    {Env, Opts} = xqerl_test:handle_environment(environment('map', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "map-merge-026.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
