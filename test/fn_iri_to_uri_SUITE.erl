...
'fn-iri-to-uri1args-1'(Config) ->
   ...
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri1args-1.xq"), Qry1),
...
'fn-iri-to-uri1args-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri1args-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-iri-to-uri1args-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri1args-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-iri-to-uri1args-4'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri1args-4.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri1args-5'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri1args-5.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-iri-to-uri1args-6'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri1args-6.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-iri-to-uri-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-iri-to-uri-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-iri-to-uri-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-iri-to-uri-4'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-4.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example-example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-5'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-5.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'fn-iri-to-uri-6'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-6.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example.example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-7'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-7.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example!example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-8'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-8.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example~example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-9'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-9.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example*example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-10'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-10.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example'example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-11'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-11.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example(example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-12'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-12.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example)example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-13'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-13.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example;example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-14'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-14.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example/example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-15'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-15.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example?example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-16'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-16.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example:example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-17'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-17.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example@example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-18'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-18.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example&amp;example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-19'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-19.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example=example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-20'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-20.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example+example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-21'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-21.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example$example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-22'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-22.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example,example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-23'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-23.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example[example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-24'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-24.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example]example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-25'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-25.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "example%example") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'fn-iri-to-uri-26'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "fn-iri-to-uri-26.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K-IRIToURIfunc-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K-IRIToURIfunc-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XPST0017") of 
      true -> {comment, "Correct error"};
      {false, F} -> F 
...
'K-IRIToURIfunc-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K-IRIToURIfunc-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XPST0017") of 
      true -> {comment, "Correct error"};
      {false, F} -> F 
...
'K-IRIToURIfunc-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K-IRIToURIfunc-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K-IRIToURIfunc-4'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K-IRIToURIfunc-4.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K2-IRIToURIfunc-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K2-IRIToURIfunc-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XPST0017") of 
      true -> {comment, "Correct error"};
      {false, F} -> F 
...
'K2-IRIToURIfunc-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K2-IRIToURIfunc-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XPST0017") of 
      true -> {comment, "Correct error"};
      {false, F} -> F 
...
'K2-IRIToURIfunc-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K2-IRIToURIfunc-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XPTY0004") of 
      true -> {comment, "Correct error"};
      {false, F} -> F 
...
'K2-IRIToURIfunc-4'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K2-IRIToURIfunc-4.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XPTY0004") of 
      true -> {comment, "Correct error"};
      {false, F} -> F 
...
'K2-IRIToURIfunc-5'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K2-IRIToURIfunc-5.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K2-IRIToURIfunc-6'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K2-IRIToURIfunc-6.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "a%20string") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'K2-IRIToURIfunc-7'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K2-IRIToURIfunc-7.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
\")", 
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K2-IRIToURIfunc-8.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K2-IRIToURIfunc-9'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K2-IRIToURIfunc-9.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K2-IRIToURIfunc-10'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_module:compile(filename:join(__BaseDir, "K2-IRIToURIfunc-10.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
