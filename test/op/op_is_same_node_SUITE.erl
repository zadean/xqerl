...
'nodeexpression1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'nodeexpression2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'nodeexpression3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression3.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression4.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'nodeexpression6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'nodeexpression7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression7.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression8.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression9.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression10.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression11.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression12.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression13.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression14.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression15.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpression16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpression16.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpressionhc1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpressionhc1.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpressionhc2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpressionhc2.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpressionhc3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpressionhc3.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpressionhc4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpressionhc4.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'nodeexpressionhc5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "nodeexpressionhc5.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "nodecomparisonerr-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NodeSame-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-NodeSame-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-NodeSame-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-NodeSame-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-NodeSame-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-NodeSame-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-NodeSame-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-NodeSame-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-NodeSame-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-NodeSame-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-NodeSame-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-NodeSame-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-NodeSame-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-NodeSame-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-NodeSame-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-NodeSame-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-NodeSame-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-NodeSame-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-NodeSame-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-NodeSame-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-NodeSame-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-NodeSame-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-NodeSame-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-NodeSame-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-node-same-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-node-same-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-node-same-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-node-same-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-is-same-node-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-is-same-node-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-is-same-node-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-is-same-node-002.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
