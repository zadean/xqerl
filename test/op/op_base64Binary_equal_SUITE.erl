    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-base64Binary-equal2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-base64Binary-equal2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-base64Binary-equal2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-base64Binary-equal2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-base64Binary-equal2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-base64Binary-equal2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-base64Binary-equal2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-base64Binary-equal2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-base64Binary-equal2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-base64Binary-equal2args-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-base64Binary-equal2args-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-base64Binary-equal2args-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-base64Binary-equal2args-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-base64Binary-equal2args-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-base64Binary-equal2args-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-base64Binary-equal2args-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-base64Binary-equal2args-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-base64Binary-equal2args-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-base64Binary-equal2args-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64binaryequal-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64binaryequal-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64BinaryEqual-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64BinaryEqual-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-Base64BinaryEQ-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-Base64BinaryEQ-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-Base64BinaryEQ-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-Base64BinaryEQ-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-Base64BinaryEQ-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-Base64BinaryEQ-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-base64Binary-equal-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-base64Binary-equal-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-base64Binary-equal-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-base64Binary-equal-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-base64Binary-equal-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-base64Binary-equal-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
