    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2970-07-01Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-yearMonthDuration-to-date2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "4001-01-01Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-yearMonthDuration-to-date-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2001-12-30") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-yearMonthDuration-to-date-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1997-11-05Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-yearMonthDuration-to-date-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-date-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-date-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DateAddYMD-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateAddYMD-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateAddYMD-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateAddYMD-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateAddYMD-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateAddYMD-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateAddYMD-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateAddYMD-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-add-yearMonthDuration-to-date-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-add-yearMonthDuration-to-date-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-add-yearMonthDuration-to-date-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-add-yearMonthDuration-to-date-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-add-yearMonthDuration-to-date-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-add-yearMonthDuration-to-date-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
