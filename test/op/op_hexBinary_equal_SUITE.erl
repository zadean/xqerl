    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-hexBinary-equal2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-hexBinary-equal2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-hexBinary-equal2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-hexBinary-equal2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-hexBinary-equal2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-hexBinary-equal2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-hexBinary-equal2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-hexBinary-equal2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-hexBinary-equal2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-hexBinary-equal2args-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-hexBinary-equal2args-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-hexBinary-equal2args-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-hexBinary-equal2args-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-hexBinary-equal2args-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-hexBinary-equal2args-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-hexBinary-equal2args-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-hexBinary-equal2args-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-hexBinary-equal2args-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-hexBinary-equal2args-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'hexBinary-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-HexBinaryEQ-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-HexBinaryEQ-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-HexBinaryEQ-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-HexBinaryEQ-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-HexBinaryEQ-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-HexBinaryEQ-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-HexBinaryEQ-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-HexBinaryEQ-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-HexBinaryEQ-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-HexBinaryEQ-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-HexBinaryEQ-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-HexBinaryEQ-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-HexBinaryEQ-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-HexBinaryEQ-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-HexBinaryEQ-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-HexBinaryEQ-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-hexBinary-equal-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-hexBinary-equal-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-hexBinary-equal-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-hexBinary-equal-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-hexBinary-equal-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-hexBinary-equal-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
