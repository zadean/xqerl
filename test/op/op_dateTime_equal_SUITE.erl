    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-dateTime-equal2args-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-dateTime-equal2args-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DateTimeEQ-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-26.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-27.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-28.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-29.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-30.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-31'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-31.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateTimeEQ-32'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateTimeEQ-32.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-DateTimeEQ-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-DateTimeEQ-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-dateTime-equal-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-008.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-009.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-010.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-011.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-012.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-013.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-015.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-016.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-017.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-dateTime-equal-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-dateTime-equal-018.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
