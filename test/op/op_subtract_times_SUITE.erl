    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "PT8H3M35S") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-subtract-times2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "PT23H59M59S") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-subtract-times2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "PT7H12M") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-subtract-times-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "-PT4H") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-subtract-times-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-times-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-times-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimeSubtract-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeSubtract-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeSubtract-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeSubtract-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeSubtract-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeSubtract-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeSubtract-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeSubtract-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeSubtract-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeSubtract-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeSubtract-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeSubtract-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeSubtract-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeSubtract-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeSubtract-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeSubtract-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-subtract-times-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-subtract-times-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-subtract-times-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-subtract-times-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-subtract-times-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-subtract-times-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-subtract-times-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-subtract-times-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
