...
'K-StringGT-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-StringGT-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-StringGT-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-StringGT-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-StringGT-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-StringGT-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-StringGT-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-StringGT-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-StringGT-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-StringGT-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-string-greater-than-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-string-greater-than-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-string-greater-than-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-string-greater-than-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-string-greater-than-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-string-greater-than-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-string-greater-than-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
