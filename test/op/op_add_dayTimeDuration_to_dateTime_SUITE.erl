    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1970-01-16T11:59:59Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-dayTimeDuration-to-dateTime2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1970-02-01T23:59:59Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-dayTimeDuration-to-dateTime-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2000-11-02T12:27:00") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-dayTimeDuration-to-dateTime-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1989-07-06T19:12:10Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-dayTimeDuration-to-dateTime-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-dayTimeDuration-to-dateTime-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-dayTimeDuration-to-dateTime-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DateTimeAddDTD-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DateTimeAddDTD-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DateTimeAddDTD-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DateTimeAddDTD-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DateTimeAddDTD-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DateTimeAddDTD-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DateTimeAddDTD-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-DateTimeAddDTD-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-add-dayTimeDuration-to-dateTime-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-add-dayTimeDuration-to-dateTime-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-add-dayTimeDuration-to-dateTime-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-add-dayTimeDuration-to-dateTime-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-add-dayTimeDuration-to-dateTime-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-add-dayTimeDuration-to-dateTime-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
