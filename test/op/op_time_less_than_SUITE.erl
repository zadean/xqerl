    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than2args-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than2args-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than2args-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than2args-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than2args-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than2args-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than2args-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than2args-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than2args-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than2args-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-time-less-than-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-time-less-than-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-TimeLT-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeLT-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeLT-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeLT-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeLT-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeLT-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeLT-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeLT-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeLT-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeLT-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-TimeLT-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-TimeLT-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-time-less-than-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-008.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-009.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-010.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-011.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-012.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-013.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-time-less-than-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-time-less-than-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
