    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "-P4M") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-subtract-yearMonthDurations-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "P2Y6M") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-subtract-yearMonthDurations-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-subtract-yearMonthDurations-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-subtract-yearMonthDurations-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationSubtract-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationSubtract-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationSubtract-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationSubtract-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationSubtract-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationSubtract-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationSubtract-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationSubtract-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationSubtract-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationSubtract-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationSubtract-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationSubtract-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationSubtract-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationSubtract-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationSubtract-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationSubtract-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationSubtract-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationSubtract-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-subtract-yearMonthDurations-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-subtract-yearMonthDurations-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-subtract-yearMonthDurations-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-subtract-yearMonthDurations-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-subtract-yearMonthDurations-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-subtract-yearMonthDurations-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
