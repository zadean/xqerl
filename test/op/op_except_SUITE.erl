    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-001.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-002.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-003.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2000") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-except-node-args-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-004.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-005.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-006.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "PI2") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-except-node-args-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-007.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-008.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-009.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "PI1 PI2") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-except-node-args-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-010.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "PI1") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-except-node-args-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-011.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-012.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-013.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-014.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-022.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-except-node-args-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-except-node-args-023.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K2-SeqExcept-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-26.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-27.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-28.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-29.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-30.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-31'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-31.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-32'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-32.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-33'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-33.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-34'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-34.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-35'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-35.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-36'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-36.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-37'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-37.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-38'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-38.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-39'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-39.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'K2-SeqExcept-40'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-40.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'K2-SeqExcept-41'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-41.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExcept-42'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExcept-42.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'combiningnodeseqexcepthc1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "combiningnodeseqexcepthc1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'combiningnodeseqexcepthc2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "combiningnodeseqexcepthc2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'combiningnodeseqexcepthc3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "combiningnodeseqexcepthc3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'combiningnodeseqexcepthc4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "combiningnodeseqexcepthc4.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'combiningnodeseqexcepthc5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "combiningnodeseqexcepthc5.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'combiningnodeseqexcepthc6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "combiningnodeseqexcepthc6.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
