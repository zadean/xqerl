    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal2args-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal2args-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal2args-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal2args-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal2args-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal2args-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal2args-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal2args-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal2args-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal2args-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "op-gMonth-equal-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'op-gMonth-equal-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "op-gMonth-equal-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'op-gMonth-equal-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "op-gMonth-equal-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'op-gMonth-equal-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "op-gMonth-equal-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'op-gMonth-equal-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "op-gMonth-equal-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'op-gMonth-equal-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "op-gMonth-equal-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'op-gMonth-equal-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "op-gMonth-equal-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'op-gMonth-equal-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "op-gMonth-equal-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'op-gMonth-equal-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-gMonth-equal-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-gMonth-equal-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-gMonthEQ-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-gMonthEQ-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-gMonthEQ-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-gMonthEQ-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-gMonthEQ-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-gMonthEQ-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-gMonthEQ-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-gMonthEQ-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-gMonthEQ-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-gMonthEQ-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-gMonthEQ-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-gMonthEQ-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-gMonthEQ-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-gMonthEQ-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-gMonthEQ-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-gMonthEQ-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-gMonth-equal-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-008.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-009.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-010.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-011.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-012.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-013.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-gMonth-equal-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-gMonth-equal-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
