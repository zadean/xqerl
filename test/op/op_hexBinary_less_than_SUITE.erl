...
'hexBinary-lt-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-11'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "(xs:hexBinary(\"786174616d61616772\") <= xs:hexBinary(\"767479716c6a647663\")) and (xs:hexBinary(\"786174616d61616772\") lt xs:hexBinary(\"767479716c6a647663\"))",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-12'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "(xs:hexBinary(\"786174616d61616772\") <= xs:hexBinary(\"767479716c6a647663\")) or (xs:hexBinary(\"786174616d61616772\") lt xs:hexBinary(\"767479716c6a647663\"))",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_eq(Res, "xs:hexBinary('88')") of
            true -> {comment, "Equal"};
            {false, F} -> F
        end,
    case Out of
...
'hexBinary-lt-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "'', '88', 'AA', 'BB', 'BBBB'") of
...
'hexBinary-lt-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-26.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-le-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-le-27.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-le-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-le-28.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-le-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-le-29.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-30.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-31'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-31.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'hexBinary-lt-32'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "hexBinary-lt-32.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
