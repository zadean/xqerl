...
'fn-intersect-node-args-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-001.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "TCP/IP Illustrated") of
...
'fn-intersect-node-args-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-002.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-intersect-node-args-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-003.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-intersect-node-args-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-004.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "Data on the Web") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-intersect-node-args-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-005.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-intersect-node-args-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-006.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-intersect-node-args-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-007.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-intersect-node-args-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-008.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-intersect-node-args-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-009.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-intersect-node-args-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-010.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-intersect-node-args-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-011.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1994 1992 2000 1999") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'fn-intersect-node-args-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-012.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-intersect-node-args-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-013.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'fn-intersect-node-args-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'fn-intersect-node-args-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "fn-intersect-node-args-023.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a> 65.95</a><a>65.95</a><a> 39.95</a><a>129.95</a>") of
...
'K2-SeqIntersect-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqIntersect-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqIntersect-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqIntersect-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqIntersect-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqIntersect-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqIntersect-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqIntersect-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqIntersect-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqIntersect-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqIntersect-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqIntersect-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqIntersect-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqIntersect-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqIntersect-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqIntersect-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqIntersect-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqIntersect-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqIntersect-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-22.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-23.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-24.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-25.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-26.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-27.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-28.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-29.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-30.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-31'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-31.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-32'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-32.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-33'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-33.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-34'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-34.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-35'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-35.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-36'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-36.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-37'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-37.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-38'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-38.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-39'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-39.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K2-SeqIntersect-40'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-40.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K2-SeqIntersect-41'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-41.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-42'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-42.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-43'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-43.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-44'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-44.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-SeqIntersect-45'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-45.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K2-SeqIntersect-46'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-46.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_eq(Res, "\"foo\"") of
            true -> {comment, "Equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-SeqIntersect-47'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-SeqIntersect-47.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'combiningnodeseqintersecthc1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "combiningnodeseqintersecthc1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'combiningnodeseqintersecthc2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "combiningnodeseqintersecthc2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'combiningnodeseqintersecthc3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "combiningnodeseqintersecthc3.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'combiningnodeseqintersecthc4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "combiningnodeseqintersecthc4.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
