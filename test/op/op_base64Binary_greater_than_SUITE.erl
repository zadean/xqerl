...
'base64Binary-gt-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base64Binary-gt-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base64Binary-gt-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base64Binary-gt-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base64Binary-gt-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base64Binary-gt-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base64Binary-gt-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base64Binary-gt-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base64Binary-gt-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base64Binary-gt-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base64Binary-gt-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base64Binary-gt-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base64Binary-gt-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base64Binary-gt-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base64Binary-gt-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base64Binary-gt-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base64Binary-gt-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base64Binary-gt-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-11'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "(xs:base64Binary(\"eGF0YW1hYWdy\") >= xs:base64Binary(\"dnR5cWxqZHZj\")) and (xs:base64Binary(\"eGF0YW1hYWdy\") gt xs:base64Binary(\"dnR5cWxqZHZj\"))",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-12'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "(xs:base64Binary(\"eGF0YW1hYWdy\") >= xs:base64Binary(\"dnR5cWxqZHZj\")) or (xs:base64Binary(\"eGF0YW1hYWdy\") gt xs:base64Binary(\"dnR5cWxqZHZj\"))",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_eq(Res, "xs:base64Binary('uw==')") of
            true -> {comment, "Equal"};
            {false, F} -> F
        end,
    case Out of
...
'base64Binary-gt-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "'u7s=', 'uw==', 'qg==', 'iA==', ''") of
...
'base64Binary-gt-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-19'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "xs:base64Binary(xs:base64Binary(xs:base64Binary(\"Aw==\"))) gt xs:base64Binary(\"Aw==\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-20'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "xs:base64Binary(xs:base64Binary(xs:base64Binary(\"AAE=\"))) ge  xs:base64Binary(\"AAI=\")",
    Qry1 = Qry,
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-22.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-23.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-24.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-25.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-26.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-ge-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-ge-27.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-ge-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-ge-28.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-ge-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-ge-29.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-30'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-30.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-31'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-31.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base64Binary-gt-32'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base64Binary-gt-32.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
