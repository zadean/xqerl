    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2970-07-01T00:00:00Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-yearMonthDuration-to-dateTime2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "4001-01-01T00:00:00Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-yearMonthDuration-to-dateTime-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2001-12-30T11:12:00") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-yearMonthDuration-to-dateTime-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTimealt-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTimealt-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1997-11-05T14:34:36Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-add-yearMonthDuration-to-dateTime-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-add-yearMonthDuration-to-dateTime-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-add-yearMonthDuration-to-dateTime-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationAddDT-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationAddDT-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationAddDT-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationAddDT-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationAddDT-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationAddDT-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationAddDT-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationAddDT-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationAddDT-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationAddDT-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-YearMonthDurationAddDT-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-YearMonthDurationAddDT-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-add-yearMonthDuration-to-dateTime-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-add-yearMonthDuration-to-dateTime-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-add-yearMonthDuration-to-dateTime-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-add-yearMonthDuration-to-dateTime-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-add-yearMonthDuration-to-dateTime-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-add-yearMonthDuration-to-dateTime-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
