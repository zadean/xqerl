...
'K-AnyURIEqual-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-AnyURIEqual-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-AnyURIEqual-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-AnyURIEqual-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-AnyURIEqual-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-AnyURIEqual-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-AnyURIEqual-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-AnyURIEqual-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-AnyURIEqual-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-AnyURIEqual-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-AnyURIEqual-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-AnyURIEqual-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-AnyURIEqual-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-AnyURIEqual-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-AnyURIEqual-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-AnyURIEqual-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-AnyURIEqual-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-AnyURIEqual-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-AnyURIEqual-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-AnyURIEqual-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-AnyURIEqual-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AnyURIEqual-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
