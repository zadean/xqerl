    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-date-equal2args-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-date-equal2args-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-DateEQ-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateEQ-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateEQ-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateEQ-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateEQ-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateEQ-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateEQ-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateEQ-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateEQ-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateEQ-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateEQ-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateEQ-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateEQ-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateEQ-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-DateEQ-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-DateEQ-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-DateEQ-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-DateEQ-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-date-eq-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-date-eq-001.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-date-ne-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-date-ne-001.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-date-equal-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-008.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-009.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-010.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-011.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-012.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-013.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-015.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-date-equal-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-date-equal-016.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
