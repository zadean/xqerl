    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "sequenceexpressionhc1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'sequenceexpressionhc2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "sequenceexpressionhc2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'sequenceexpressionhc3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "sequenceexpressionhc3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'sequenceexpressionhc4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "sequenceexpressionhc4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'sequenceexpressionhc5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "sequenceexpressionhc5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'sequenceexpressionhc6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "sequenceexpressionhc6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'sequenceexpressionhc7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "sequenceexpressionhc7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "15, 14, 13, 12, 11, 10") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'sequenceexpressionhc8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "sequenceexpressionhc8.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'sequenceexpressionhc9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "sequenceexpressionhc9.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'constSeq-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "true(), true()") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'constSeq-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "false(), false()") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'constSeq-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"ABC\", \"D\", \"E\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'constSeq-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "xs:float('1.1e0'), 2.2, 3.3") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'constSeq-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "120, 220, 330") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'constSeq-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'constSeq-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2004-12-25Z 2004-12-26Z") of
...
'constSeq-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1999-11-28T09:00:00Z 1998-11-28T09:00:00Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'constSeq-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "constSeq-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "08:00:00+09:00 08:00:00+10:00") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-concatenate-mix-args-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-concatenate-mix-args-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"a\", \"xyz\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-concatenate-mix-args-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"a\", \"www.example.com\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"hello\", 100, \"www.example.com\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "\"www.example.com\", 1.01") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-008.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1.01 NaN") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-009.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-concatenate-mix-args-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-010.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "INF -INF -INF") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-011.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "true(), false(), 0") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-012.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-concatenate-mix-args-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-013.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-concatenate-mix-args-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1972-12-31T00:00:00Z") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-015.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "12:30:00Z   2.000000000000002") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-016.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "  2.000000000000002") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-017.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'op-concatenate-mix-args-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-018.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "1, 2, 2, 1, 2, 3, 123, \"\", \"\"") of
            true -> {comment, "Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'op-concatenate-mix-args-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-019.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'op-concatenate-mix-args-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "op-concatenate-mix-args-020.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K-commaOp-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-commaOp-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-commaOp-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-commaOp-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-commaOp-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-commaOp-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
