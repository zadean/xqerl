...
{schemas, []},
{collections, []},
{'static-base-uri', [{"file:///C:/git/zadean/xquery-3.1/QT3-test-suite/fn/collation-key.xml"}]},
{params, []},
{namespaces, [{"http://www.w3.org/2005/xpath-functions/map","map"}]},
{resources, []},
{modules, []}
...
'collation-key-101'(_Config) ->
   ...
   Env = xqerl_test:handle_environment(environment('html5')),
...
'collation-key-201'(_Config) ->
   Qry = "
        ...
        collation-key(\"abc\", $c) eq collation-key(\"123\", $c)",
   Env = xqerl_test:handle_environment(environment('primary')),
   Qry1 = lists:flatten(Env ++ Qry),
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try xqerl:run(Qry1) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_false(Res) of 
