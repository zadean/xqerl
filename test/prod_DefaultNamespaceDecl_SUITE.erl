...
'default_namespace-022'(_Config) ->
   ...
   Exp = "
      ...
      ",
   case catch xqerl_node:to_xml(xqerl_test:run(case xqerl_node:to_xml(Res) of {xqError,_,_,_,_} -> "deep-equal(<x></x>"; P -> "deep-equal(<x>"++P++"</x>" end ++ " , " ++ "<x>"++"<test xmlns=\"http://www.example.com/test\"/>"++"</x>)")) == "true" of
           true -> {comment, "assert-xml"};
           _ -> 
...
'K2-DefaultNamespaceProlog-6'(_Config) ->
   ...
   Exp = "
      ...
      ",
   case catch xqerl_node:to_xml(xqerl_test:run(case xqerl_node:to_xml(Res) of {xqError,_,_,_,_} -> "deep-equal(<x></x>"; P -> "deep-equal(<x>"++P++"</x>" end ++ " , " ++ "<x>"++"http://example.com/<e> |  </e>http://example.com/"++"</x>)")) == "true" of
           true -> {comment, "assert-xml"};
           _ -> 
...
'K2-DefaultNamespaceProlog-11'(_Config) ->
   ...
   Exp = "
      ...
      ",
   case catch xqerl_node:to_xml(xqerl_test:run(case xqerl_node:to_xml(Res) of {xqError,_,_,_,_} -> "deep-equal(<x></x>"; P -> "deep-equal(<x>"++P++"</x>" end ++ " , " ++ "<x>"++"<a xmlns=\"http://www.w3.org/2001/XMLSchema\">1<b xmlns=\"http://www.w3.org/1999/XSL/Transform\">1</b>2</a>"++"</x>)")) == "true" of
           true -> {comment, "assert-xml"};
           _ -> 
