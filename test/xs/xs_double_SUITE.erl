...
'xs-double-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "xs-double-004.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'xs-double-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "xs-double-005.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
