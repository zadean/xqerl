...
'Constr-comppi-name-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-name-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<?pi text?>") of
...
'Constr-comppi-name-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-name-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-4.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'Constr-comppi-compname-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-compname-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-compname-22.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-doc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-doc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<?pi texttext texttext?>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-comppi-parent-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-parent-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-string-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-string-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-data-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-data-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-enclexpr-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-enclexpr-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case
            xqerl_test:assert_xml(Res, "<?pi 1 string 3.14 0.012345 2002-04-02T12:00:00-01:00?>")
        of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
...
'Constr-comppi-enclexpr-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-enclexpr-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<?pi 123 456 789?>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-comppi-enclexpr-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-enclexpr-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<?pi 1  2?>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-comppi-enclexpr-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-enclexpr-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-enclexpr-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-enclexpr-5.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<?pi texttext?>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-comppi-namexml-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-namexml-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-namexml-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-namexml-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-namexml-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-namexml-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-namexml-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-namexml-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-invalid-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-invalid-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-invalid-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-invalid-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-invalid-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-invalid-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-space-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-space-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-space-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-space-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-space-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-space-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "116 101 120 116") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-comppi-space-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-space-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-empty-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-empty-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-comppi-empty-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-comppi-empty-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ComputeConPI-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConPI-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ComputeConPI-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConPI-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ComputeConPI-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConPI-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ComputeConPI-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConPI-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ComputeConPI-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConPI-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "XmLnaMe") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ComputeConPI-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConPI-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ComputeConPI-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConPI-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ComputeConPI-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConPI-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "asdas? >") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ComputeConPI-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConPI-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "content {1+ } {") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ComputeConPI-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ComputeConPI-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ComputeConPI-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ComputeConPI-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-constr-comppi-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-constr-comppi-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-constr-comppi-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-constr-comppi-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
