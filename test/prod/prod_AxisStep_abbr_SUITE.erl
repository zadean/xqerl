...
'abbreviatedSyntax-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-1.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "20 40") of
...
'abbreviatedSyntax-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-2.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-3.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "John Doe 10") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'abbreviatedSyntax-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-5.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-6.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-7.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case
            xqerl_test:assert_string_value(Res, "40 70 20 80 20 40 20 30 12 40 80 20 20 20 40 80")
        of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
...
'abbreviatedSyntax-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-8.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-9.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-10.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-12.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "Monday Tuesday") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'abbreviatedSyntax-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-13.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-14.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-16.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "Jane Doe 11") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'abbreviatedSyntax-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-17.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "Jane Doe 9") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'abbreviatedSyntax-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-18.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "Jane Doe 5") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'abbreviatedSyntax-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-19.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-20.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-21.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-22.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-24.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-25.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'abbreviatedSyntax-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "abbreviatedSyntax-26.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-AbbrAxes-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-AbbrAxes-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
