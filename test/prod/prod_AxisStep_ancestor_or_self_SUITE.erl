...
'ancestorself-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'ancestorself-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-2.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-3.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-4.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-5.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-6.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-7.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-8.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-9.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-10.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-11.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-12.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-13.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "John Doe 12") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'ancestorself-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-14.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-15.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-16.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-17.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-18.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-19.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-20.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestorself-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestorself-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<anElement>Some Content</anElement>") of
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "unabbreviatedSyntax-11.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K2-ancestor-or-selfAxis-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestor-or-selfAxis-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<out>1<element/>1</out>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ancestor-or-selfAxis-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestor-or-selfAxis-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<out>1<?target data?>1</out>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ancestor-or-selfAxis-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestor-or-selfAxis-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "1, true(), 1") of
...
'K2-ancestor-or-selfAxis-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestor-or-selfAxis-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<out>1<!-- content -->1</out>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ancestor-or-selfAxis-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestor-or-selfAxis-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestor-or-selfAxis-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestor-or-selfAxis-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestor-or-selfAxis-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestor-or-selfAxis-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestor-or-selfAxis-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestor-or-selfAxis-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1 2 3 2") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ancestor-or-selfAxis-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestor-or-selfAxis-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<r><a><b><c/></b></a></r><b><c/></b><c/>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
