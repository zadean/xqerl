    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'followingsibling-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-2.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-3.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-4.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-5.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-6.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-7.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-8.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-9.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-10.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-11.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-12.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-13.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-14.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-15.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-16.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-17.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-18.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-19.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-20.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'followingsibling-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "followingsibling-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'unabbreviatedSyntax-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "unabbreviatedSyntax-24.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K2-following-siblingAxis-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-following-siblingAxis-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<child/><child/>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-following-siblingAxis-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-following-siblingAxis-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<child/><child attr=\"foo\" attr2=\"foo\"/>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-following-siblingAxis-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-following-siblingAxis-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-following-siblingAxis-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-following-siblingAxis-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "0 2 1 0 0 0 0") of
...
'K2-following-siblingAxis-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-following-siblingAxis-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-following-siblingAxis-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-following-siblingAxis-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-following-siblingAxis-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-following-siblingAxis-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-following-siblingAxis-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-following-siblingAxis-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-following-siblingAxis-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-following-siblingAxis-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-following-siblingAxis-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-following-siblingAxis-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'following-sibling-attr'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "following-sibling-attr.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
