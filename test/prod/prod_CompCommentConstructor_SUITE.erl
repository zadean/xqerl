...
'Constr-compcomment-enclexpr-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-enclexpr-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case
            xqerl_test:assert_xml(Res, "<!--1 string 3.14 0.012345 2002-04-02T12:00:00-01:00-->")
        of
...
'Constr-compcomment-enclexpr-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-enclexpr-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<!--123 456 789-->") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-compcomment-enclexpr-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-enclexpr-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<!--1  2-->") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-compcomment-enclexpr-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-enclexpr-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-enclexpr-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-enclexpr-5.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<!--texttext-->") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-compcomment-doc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-doc-1.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<!--texttext texttext-->") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-compcomment-parent-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-parent-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-string-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-string-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-data-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-data-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-nested-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-nested-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<!--one two three four-->") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-compcomment-nested-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-nested-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-nested-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-nested-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-dash-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-dash-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-dash-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-dash-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-dash-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-dash-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-dash-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-dash-4.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'Constr-compcomment-doubledash-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-doubledash-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-doubledash-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-doubledash-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-doubledash-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-doubledash-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-compcomment-doubledash-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-doubledash-4.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'Constr-compcomment-doubledash-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-compcomment-doubledash-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<!--com - - ment-->") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ComputeConComment-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ComputeConComment-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ComputeConComment-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ComputeConComment-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ComputeConComment-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ComputeConComment-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ComputeConComment-4a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ComputeConComment-4a.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ComputeConComment-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ComputeConComment-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-constr-compcomment-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-constr-compcomment-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-constr-compcomment-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-constr-compcomment-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
