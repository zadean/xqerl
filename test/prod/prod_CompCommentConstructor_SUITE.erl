...
'Constr-compcomment-enclexpr-1'(Config) ->
   ...
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-enclexpr-1.xq"), Qry1),
...
'Constr-compcomment-enclexpr-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-enclexpr-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'Constr-compcomment-enclexpr-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-enclexpr-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'Constr-compcomment-enclexpr-4'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-enclexpr-4.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_xml(Res,"<!--1  2-->") of 
      true -> {comment, "XML Deep equal"};
      {false, F} -> F 
...
'Constr-compcomment-enclexpr-5'(Config) ->
   ...
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-enclexpr-5.xq"), Qry1),
...
'Constr-compcomment-doc-1'(Config) ->
   ...
   Qry1 = lists:flatten(Env ++ Qry),
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-doc-1.xq"), Qry1),
             xqerl:run(Mod,Opts) of D -> D catch _:E -> E end,
...
'Constr-compcomment-parent-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-parent-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'Constr-compcomment-string-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-string-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'Constr-compcomment-data-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-data-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_string_value(Res, "a  b") of 
      true -> {comment, "String correct"};
      {false, F} -> F 
...
'Constr-compcomment-nested-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-nested-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'Constr-compcomment-nested-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-nested-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'Constr-compcomment-nested-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-nested-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'Constr-compcomment-dash-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-dash-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'Constr-compcomment-dash-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-dash-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XQDY0072") of 
      true -> {comment, "Correct error"};
      {true, F} -> {comment, "WE: XQDY0072 " ++ binary_to_list(F)};
...
'Constr-compcomment-dash-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-dash-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XQDY0072") of 
      true -> {comment, "Correct error"};
      {true, F} -> {comment, "WE: XQDY0072 " ++ binary_to_list(F)};
...
{modules, []}
   ...
   Qry1 = lists:flatten(Env ++ Qry),
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-dash-4.xq"), Qry1),
             xqerl:run(Mod,Opts) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XQDY0072") of 
      true -> {comment, "Correct error"};
      {true, F} -> {comment, "WE: XQDY0072 " ++ binary_to_list(F)};
...
'Constr-compcomment-doubledash-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-doubledash-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XQDY0072") of 
      true -> {comment, "Correct error"};
      {true, F} -> {comment, "WE: XQDY0072 " ++ binary_to_list(F)};
...
'Constr-compcomment-doubledash-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-doubledash-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XQDY0072") of 
      true -> {comment, "Correct error"};
      {true, F} -> {comment, "WE: XQDY0072 " ++ binary_to_list(F)};
...
'Constr-compcomment-doubledash-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-doubledash-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XQDY0072") of 
      true -> {comment, "Correct error"};
      {true, F} -> {comment, "WE: XQDY0072 " ++ binary_to_list(F)};
...
{modules, []}
   ...
   Qry1 = lists:flatten(Env ++ Qry),
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-doubledash-4.xq"), Qry1),
             xqerl:run(Mod,Opts) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XQDY0072") of 
      true -> {comment, "Correct error"};
      {true, F} -> {comment, "WE: XQDY0072 " ++ binary_to_list(F)};
...
'Constr-compcomment-doubledash-5'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-compcomment-doubledash-5.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K2-ComputeConComment-1'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConComment-1.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K2-ComputeConComment-2'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConComment-2.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XPST0003") of 
      true -> {comment, "Correct error"};
      {true, F} -> {comment, "WE: XPST0003 " ++ binary_to_list(F)};
...
'K2-ComputeConComment-3'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConComment-3.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K2-ComputeConComment-4a'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConComment-4a.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'K2-ComputeConComment-5'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ComputeConComment-5.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
...
'cbcl-constr-compcomment-001'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-constr-compcomment-001.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XQDY0072") of 
      true -> {comment, "Correct error"};
      {true, F} -> {comment, "WE: XQDY0072 " ++ binary_to_list(F)};
...
'cbcl-constr-compcomment-002'(Config) ->
   ...
   Qry1 = Qry,
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-constr-compcomment-002.xq"), Qry1),
             xqerl:run(Mod) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_error(Res,"XQDY0072") of 
      true -> {comment, "Correct error"};
      {true, F} -> {comment, "WE: XQDY0072 " ++ binary_to_list(F)};
