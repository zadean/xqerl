    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a> z abc</a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'boundary-space-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-007.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-008.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-009.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-010.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-011.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-012.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-013.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-014.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-015.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-016.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-017.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-018.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-019.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<A> (a), (b), (c) </A>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'boundary-space-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-020.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundary-space-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-021.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "  10 20 30 40  ") of
...
'boundary-space-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundary-space-022.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'boundaryspacedeclerr-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "boundaryspacedeclerr-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-BoundarySpaceProlog-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-BoundarySpaceProlog-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-BoundarySpaceProlog-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-BoundarySpaceProlog-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-BoundarySpaceProlog-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-BoundarySpaceProlog-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-BoundarySpaceProlog-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-BoundarySpaceProlog-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-BoundarySpaceProlog-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-BoundarySpaceProlog-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
