...
'typeswitchhc1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<wrap>test passed - integer data type</wrap>") of
...
'typeswitchhc2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<wrap>test passed - 5.1 is a decimal type</wrap>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'typeswitchhc3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<wrap>test passed - 5.1E2 is a double type</wrap>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'typeswitchhc4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case
            xqerl_test:assert_xml(Res, "<wrap>test passed - \"A String\" is a string type</wrap>")
        of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
...
'typeswitchhc5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case
            xqerl_test:assert_xml(Res, "<wrap>test passed - 1267.43233E12 is a double type</wrap>")
        of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
...
'typeswitchhc6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<wrap>test passed - 1 &gt; 2 is a boolean type</wrap>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'typeswitchhc7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "4.2") of
...
'typeswitchhc19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_eq(Res, "620") of
            true -> {comment, "Equal"};
            {false, F} -> F
        end,
    case Out of
...
'typeswitchhc20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'typeswitchhc25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitchhc25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-sequenceExprTypeswitch-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-sequenceExprTypeswitch-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-sequenceExprTypeswitch-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-sequenceExprTypeswitch-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-sequenceExprTypeswitch-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-sequenceExprTypeswitch-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-sequenceExprTypeswitch-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-sequenceExprTypeswitch-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-sequenceExprTypeswitch-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-sequenceExprTypeswitch-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-sequenceExprTypeswitch-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-sequenceExprTypeswitch-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-sequenceExprTypeswitch-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-sequenceExprTypeswitch-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-sequenceExprTypeswitch-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-sequenceExprTypeswitch-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-sequenceExprTypeswitch-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'typeswitch-union-nomatch'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "typeswitch-union-nomatch.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'typeswitch-union-nomatch-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "typeswitch-union-nomatch-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'typeswitch-union-branch-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "typeswitch-union-branch-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'typeswitch-union-branch-1-dup'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "typeswitch-union-branch-1-dup.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'typeswitch-union-branch-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "typeswitch-union-branch-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'typeswitch-union-branch-2-dup'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "typeswitch-union-branch-2-dup.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'typeswitch-union-branch-both'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "typeswitch-union-branch-both.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'typeswitch-union-branch-both-dup'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "typeswitch-union-branch-both-dup.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'typeswitch-union-multi'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "typeswitch-union-multi.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_deep_eq(Res, "1, 1, 0") of
...
'typeswitch-117'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitch-117.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'typeswitch-118'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitch-118.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'typeswitch-119'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "typeswitch-119.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'cbcl-typeswitch-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-typeswitch-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "INT") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'cbcl-typeswitch-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-typeswitch-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-typeswitch-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-typeswitch-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-typeswitch-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-typeswitch-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-typeswitch-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-typeswitch-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-typeswitch-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-typeswitch-006.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
