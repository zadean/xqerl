    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'preceding-sibling-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-2.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-3.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-4.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-5.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-6.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-7.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-8.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-9.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-10.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-11.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-12.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-13.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-14.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-15.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-16.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-17.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-18.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-19.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-20.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'preceding-sibling-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'unabbreviatedSyntax-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "unabbreviatedSyntax-25.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K2-preceding-siblingAxis-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-preceding-siblingAxis-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-preceding-siblingAxis-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-preceding-siblingAxis-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-preceding-siblingAxis-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-preceding-siblingAxis-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-preceding-siblingAxis-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-preceding-siblingAxis-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-preceding-siblingAxis-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-preceding-siblingAxis-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "0 0 0 1 0 1 0 2 2 1") of
...
'preceding-sibling-attr'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "preceding-sibling-attr.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
