...
'treat-as-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "String 1String 2") of
...
'treat-as-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "100 100 100") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'treat-as-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "100 100 100 100") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'treat-as-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'treat-as-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "treat-as-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-SeqExprTreat-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-SeqExprTreat-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExprTreat-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExprTreat-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExprTreat-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExprTreat-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExprTreat-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExprTreat-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-SeqExprTreat-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-SeqExprTreat-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-001.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2 4 6 8 10") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'cbcl-treat-as-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-002.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-003.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-004.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-005.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-006.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-007.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-008.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "2 4") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'cbcl-treat-as-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-009.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-010.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-011.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-012.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-013.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-014.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-015.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-016.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-017.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-018.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-019.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-020.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-021.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-022.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'cbcl-treat-as-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "cbcl-treat-as-023.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
