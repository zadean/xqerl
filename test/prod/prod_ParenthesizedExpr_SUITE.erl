...
'Parenexpr-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-15.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Parenexpr-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-16.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Parenexpr-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Parenexpr-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<elem attr=\"1\"><child/><child/></elem>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'Parenexpr-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-19.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert(Res, "name($result/*) = \"far-north\"") of
...
'Parenexpr-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Parenexpr-20.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
