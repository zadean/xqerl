...
'base-URI-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
            ...
            lists:any(
                ...
                [
                    ...
                    case xqerl_test:assert_error(Res, "FONS0005") of
                        true -> {comment, "Correct error"};
                        {true, F} -> {comment, "WE: FONS0005 " ++ binary_to_list(F)};
                        {false, F} -> F
                    end
                ]
            )
...
'base-URI-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.com/abc123") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'base-URI-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://A") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'base-URI-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.base-uri.com") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'base-URI-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-URI-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-URI-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-uri-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-uri-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.org/%20%20examples") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'base-uri-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-uri-26.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-uri-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-uri-27.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.com/fluster") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'base-uri-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-uri-28.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'base-uri-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "base-uri-29.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "http://www.example.com/fluster/now") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K-BaseURIProlog-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-BaseURIProlog-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-BaseURIProlog-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-BaseURIProlog-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-BaseURIProlog-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-BaseURIProlog-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-BaseURIProlog-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-BaseURIProlog-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-BaseURIProlog-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-BaseURIProlog-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-BaseURIProlog-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-BaseURIProlog-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-BaseURIProlog-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
            ...
            lists:any(
                ...
                [
                    ...
                    case xqerl_test:assert_error(Res, "XPST0001") of
                        true -> {comment, "Correct error"};
                        {true, F} -> {comment, "WE: XPST0001 " ++ binary_to_list(F)};
                        {false, F} -> F
                    end
                ]
            )
...
'K2-BaseURIProlog-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-BaseURIProlog-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<e xml:base=\"../\">http://example.com/BASEURI</e>") of
...
'K2-BaseURIProlog-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-BaseURIProlog-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'base-URI-modules-001'(Config) ->
    ...
    io:format("Qry1: ~p~n", [Qry1]),
    Hints = [
        {filename:join(__BaseDir, "BaseURIDecl/module-001.xq"),
            <<"Q{http://www.w3.org/TestModules/module-001}">>}
    ],
    LibList = xqerl_code_server:compile_files(Hints),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "base-URI-modules-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
