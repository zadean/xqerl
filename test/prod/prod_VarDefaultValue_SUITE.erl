...
'extvardef-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-001.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>0</a>") of
...
'extvardef-001a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-001a.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'extvardef-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-002.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-002a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-002a.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>5</a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'extvardef-002b'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-002b.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'extvardef-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-003.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>3</a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'extvardef-003a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-003a.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>15</a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'extvardef-003b'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-003b.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>42</a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'extvardef-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-004.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-005.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-006.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<out><a>17</a></out>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'extvardef-006a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-006a.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'extvardef-006b'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-006b.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'extvardef-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-007.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-008.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-009.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-010.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-010a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-010a.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-011.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-011a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-011a.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-012.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-013.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'extvardef-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-014.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'extvardef-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-015.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-016a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-016a.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-016b'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-016b.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-017.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-018.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-019.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-020.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extvardef-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-021.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_eq(Res, "fn:true()") of
            true -> {comment, "Equal"};
            {false, F} -> F
        end,
    case Out of
...
'extvardef-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extvardef-022.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
