...
'Constr-text-enclexpr-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-enclexpr-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case
            xqerl_test:assert_string_value(Res, "1 string 3.14 0.012345 2002-04-02T12:00:00-01:00")
        of
...
'Constr-text-enclexpr-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-enclexpr-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "123 456 789") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-text-enclexpr-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-enclexpr-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-enclexpr-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-enclexpr-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-enclexpr-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-enclexpr-5.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "texttext") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-text-doc-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Constr-text-doc-1.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "texttext texttext") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-text-parent-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-parent-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-string-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-string-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-data-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-data-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-count-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-count-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-count-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-count-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-count-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-count-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-count-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-count-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-count-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-count-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-count-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-count-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-adjtext-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-adjtext-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-adjtext-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-adjtext-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'Constr-text-nested-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-nested-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "one two three four") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'Constr-text-nested-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-nested-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<elem>one<a>two</a>three</elem>") of
...
'Constr-text-nested-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "Constr-text-nested-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "one<a/>two<b/>three") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ConText-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-5a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-5a.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<e>content]</e>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ConText-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ConText-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ConText-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
