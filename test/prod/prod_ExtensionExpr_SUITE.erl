...
'extexpr-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-1.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'extexpr-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-3.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'extexpr-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-5.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'extexpr-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-6.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'extexpr-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "passed") of
...
'extexpr-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'extexpr-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "extexpr-26.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ExtensionExpression-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ExtensionExpression-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ExtensionExpression-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ExtensionExpression-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ExtensionExpression-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ExtensionExpression-4a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ExtensionExpression-4a.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ExtensionExpression-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ExtensionExpression-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ExtensionExpression-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ExtensionExpression-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ExtensionExpression-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ExtensionExpression-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-ExtensionExpression-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-ExtensionExpression-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ExtensionExpression-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ExtensionExpression-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'pragma-no-namespace-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "pragma-no-namespace-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'pragma-no-namespace-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "pragma-no-namespace-002.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'pragma-no-namespace-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "pragma-no-namespace-003.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'pragma-no-namespace-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "pragma-no-namespace-004.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'pragma-no-namespace-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "pragma-no-namespace-005.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
