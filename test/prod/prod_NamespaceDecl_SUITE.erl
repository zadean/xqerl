...
'namespaceDecl-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'namespaceDecl-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "namespaceDecl-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NamespaceProlog-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NamespaceProlog-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NamespaceProlog-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-NamespaceProlog-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-NamespaceProlog-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-9.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<e xmlns:p=\"http://example.com/asd\" a=\"\"/>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-NamespaceProlog-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-NamespaceProlog-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-NamespaceProlog-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'cbcl-declare-namespace-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "cbcl-declare-namespace-001.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
