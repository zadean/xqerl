...
'preceding-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'preceding-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-2.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-3.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-4.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-5.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-6.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-7.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-8.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-9.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-10.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-11.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<day>Monday</day>") of
...
'preceding-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-12.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-13.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-14.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-15.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-16.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-17.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-18.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-19.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-20.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'preceding-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-22.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'preceding-23'(Config) ->
    ...
    {Env, Opts} = xqerl_test:handle_environment([
        {'decimal-formats', []},
        {sources, [
            {filename:join(__BaseDir, "AxisStep.preceding/AttributesAndElements.xml"), ".", []}
        ]},
        {collections, []},
        {'static-base-uri', []},
        {'context-item', [""]},
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "preceding-23.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-precedingAxis-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-precedingAxis-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-precedingAxis-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-precedingAxis-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-precedingAxis-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<preceding1/>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-precedingAxis-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-precedingAxis-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<preceding2/>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-precedingAxis-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-precedingAxis-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-precedingAxis-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-precedingAxis-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-precedingAxis-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-precedingAxis-7.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-precedingAxis-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-precedingAxis-8.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "0 0 0 1 1 2 2 4 6 8") of
