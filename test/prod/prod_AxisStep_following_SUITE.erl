...
'following-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'following-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-2.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-3.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-4.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-5.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-6.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-7.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-8.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-9.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-10.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-11.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-12.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-13.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-14.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-15.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-16.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-17.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-18.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-19.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-20.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'following-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'following-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "following-22.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'K2-followingAxis-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-followingAxis-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<p attr=\"boo\"/><p/><p/><p/>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-followingAxis-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-followingAxis-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-followingAxis-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-followingAxis-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-followingAxis-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-followingAxis-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "0 5 4 0 0 0 0") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
