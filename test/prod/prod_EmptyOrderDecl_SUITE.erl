...
'emptyorderdecl-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'emptyorderdecl-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>1</a><a>4</a><a>7</a><a/>") of
...
'emptyorderdecl-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>1</a><a>4</a><a>7</a><a/><a/>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1 4 7 NaN") of
...
'emptyorderdecl-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1 4 7 NaN NaN") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>1</a><a>4</a><a/><a>7</a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'emptyorderdecl-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1 4 NaN 7") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'emptyorderdecl-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-10.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a/><a>7</a><a>4</a><a>1</a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-11.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a/><a/><a>7</a><a>4</a><a>1</a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-12.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "NaN 7 4 1") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-13.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "NaN NaN 7 4 1") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-14.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a/><a>1</a><a>4</a><a>7</a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a/><a/><a>1</a><a>4</a><a>7</a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "NaN 1 4 7") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "NaN NaN 1 4 7") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>7</a><a>4</a><a>1</a><a/>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-19.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a>7</a><a>4</a><a>1</a><a/><a/>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-20.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "7 4 1 NaN") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "7 4 1 NaN NaN") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-22'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-22.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'emptyorderdecl-23'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-23.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "NaN 1 3 4 7") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-24'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-24.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'emptyorderdecl-25'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-25.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "1 3 4 7 NaN") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'emptyorderdecl-26'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-26.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'emptyorderdecl-27'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-27.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'emptyorderdecl-28'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-28.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'emptyorderdecl-29'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "emptyorderdecl-29.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-EmptyOrderProlog-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-EmptyOrderProlog-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-EmptyOrderProlog-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-EmptyOrderProlog-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K-EmptyOrderProlog-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
