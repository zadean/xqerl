...
'Serialization-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-002.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-003'(Config) ->
    ...
    io:format("Qry1: ~p~n", [Qry1]),
    Hints = [
        {filename:join(__BaseDir, "Serialization/serialization1-lib.xq"),
            <<"Q{http://www.w3.org/TestModules/test}">>}
    ],
    LibList = xqerl_code_server:compile_files(Hints),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-003.xq"), Qry1),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_error(Res, "XQST0108") of
            ...
            {true, F} -> {comment, "WE: XQST0108 " ++ binary_to_list(F)};
            {false, F} -> F
        end,
    case Out of
        {comment, C} -> {comment, C};
...
'Serialization-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-004.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_error(Res, "XQST0110") of
            true -> {comment, "Correct error"};
            {true, F} -> {comment, "WE: XQST0110 " ++ binary_to_list(F)};
            {false, F} -> F
        end,
    case Out of
        {comment, C} -> {comment, C};
...
'Serialization-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-005.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_error(Res, "XQST0109") of
            true -> {comment, "Correct error"};
            {true, F} -> {comment, "WE: XQST0109 " ++ binary_to_list(F)};
            {false, F} -> F
        end,
    case Out of
        {comment, C} -> {comment, C};
...
'Serialization-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-006.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
            ...
            lists:any(
                ...
                [
                    ...
                    case xqerl_test:assert_error(Res, "XQST0119") of
                        true -> {comment, "Correct error"};
                        {true, F} -> {comment, "WE: XQST0119 " ++ binary_to_list(F)};
                        {false, F} -> F
                    end
                ]
            )
...
'Serialization-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-007.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-008.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-009.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-010.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-011.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-012.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-013.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-014.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-015.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-016.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-017.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-018.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-019.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-020.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-021.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-022.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-023.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-024'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-024.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-025'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-025.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-026'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-026.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-027'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-027.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-028'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-028.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-029'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-029.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-030'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-030.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-031'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-031.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_serialization_error(Res, "SEPM0009") of
            true -> {comment, "Correct error"};
            {false, F} -> F
        end,
    case Out of
...
'Serialization-032'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-032.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-033'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-033.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-034'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-034.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-035'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-035.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case
            lists:all(
                ...
                [
                    case
                        xqerl_test:assert_serialization_match(Res, <<"CDATA\\[ta\\]"/utf8>>, <<"">>)
                    of
                        true -> {comment, "Correct serialization"};
                        {false, F} -> F
                    end,
                    case
                        xqerl_test:assert_serialization_match(Res, <<"CDATA\\[tb\\]"/utf8>>, <<"">>)
                    of
                        true -> {comment, "Correct serialization"};
                        {false, F} -> F
                    end,
                    case
                        xqerl_test:assert_serialization_match(Res, <<"CDATA\\[tc\\]"/utf8>>, <<"">>)
                    of
                        true -> {comment, "Correct serialization"};
                        {false, F} -> F
                    end,
                    case
                        xqerl_test:assert_serialization_match(Res, <<"CDATA\\[td\\]"/utf8>>, <<"">>)
                    of
                        true -> {comment, "Correct serialization"};
                        {false, F} -> F
                    end,
...
'Serialization-036'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-036.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Serialization-037'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Serialization-037.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
