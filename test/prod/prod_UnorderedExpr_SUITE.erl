...
'Orderexpr-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-1.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Orderexpr-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-2.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Orderexpr-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-5.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Orderexpr-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-6.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Orderexpr-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-9.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
            ...
            lists:all(
                ...
                [
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "some $e in $result satisfies $e/self::part[@partid=\"1\"][@partof=\"0\"][@name=\"engine\"]"
                        )
                    of
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "some $e in $result satisfies $e/self::part[@partid=\"3\"][@partof=\"1\"][@name=\"piston\"]"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
...
'Orderexpr-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-10.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Orderexpr-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-11.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Orderexpr-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-12.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Orderexpr-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-13.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Orderexpr-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-14.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Orderexpr-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-15.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Orderexpr-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-16.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Orderexpr-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-17.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Orderexpr-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-18.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'Orderexpr-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-19.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'Orderexpr-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "Orderexpr-20.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'orderedunorderedexpr-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "orderedunorderedexpr-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "0 1 2 3 4") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'orderedunorderedexpr-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "orderedunorderedexpr-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'orderedunorderedexpr-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "orderedunorderedexpr-3.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'orderedunorderedexpr-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "orderedunorderedexpr-4.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'orderedunorderedexpr-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "orderedunorderedexpr-5.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'orderedunorderedexpr-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "orderedunorderedexpr-6.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K-OrderExpr-1a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-OrderExpr-1a.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-OrderExpr-2a'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-OrderExpr-2a.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-OrderExpr-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-OrderExpr-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K-OrderExpr-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K-OrderExpr-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
