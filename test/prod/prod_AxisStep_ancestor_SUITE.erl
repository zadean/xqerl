...
'ancestor-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'ancestor-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-2.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-3.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-4.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-5.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-6.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-7.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-8.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-9.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-10.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-11.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-12.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-13.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-14.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-15.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-16.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-17.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-18.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-19.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-20.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'ancestor-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "ancestor-21.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "unabbreviatedSyntax-10.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'K2-ancestorAxis-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ancestorAxis-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ancestorAxis-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ancestorAxis-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ancestorAxis-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ancestorAxis-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ancestorAxis-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ancestorAxis-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ancestorAxis-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ancestorAxis-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ancestorAxis-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ancestorAxis-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'K2-ancestorAxis-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ancestorAxis-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a><b c=\"\"><c/></b><d/></a><b c=\"\"><c/></b>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ancestorAxis-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ancestorAxis-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<b c=\"\"><c/></b>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ancestorAxis-9'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "K2-ancestorAxis-9.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<a><b c=\"\"><c/></b><d/></a>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ancestorAxis-10'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-10.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestorAxis-11'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-11.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestorAxis-12'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-12.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestorAxis-13'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-13.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestorAxis-14'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-14.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestorAxis-15'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-15.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestorAxis-16'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-16.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestorAxis-17'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-17.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestorAxis-18'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-18.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestorAxis-19'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-19.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'K2-ancestorAxis-20'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-20.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "0 1 2 1") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'K2-ancestorAxis-21'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "K2-ancestorAxis-21.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<r><a><b><c/></b></a></r><a><b><c/></b></a><b><c/></b>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
