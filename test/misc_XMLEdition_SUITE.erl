...
'XML10-3ed-Mixed-content'(_Config) ->
   ...
   Exp = "
      ...
      ",
   case catch xqerl_node:to_xml(xqerl_test:run(case xqerl_node:to_xml(Res) of {xqError,_,_,_,_} -> "deep-equal(<x></x>"; P -> "deep-equal(<x>"++P++"</x>" end ++ " , " ++ "<x>"++"<foo> a cdata section in mixed content. a <!-- comment --> in mixed content. a <?processing instruction?> in mixed content. </foo>"++"</x>)")) == "true" of
...
'XML10-5ed-Included-char-1'(_Config) ->
   ...
   Exp = "
         <foo>&#x7f;</foo>
      ",
   case catch xqerl_node:to_xml(xqerl_test:run(case xqerl_node:to_xml(Res) of {xqError,_,_,_,_} -> "deep-equal(<x></x>"; P -> "deep-equal(<x>"++P++"</x>" end ++ " , " ++ "<x>"++"<foo>&#x7f;</foo>"++"</x>)")) == "true" of
           true -> {comment, "assert-xml"};
           _ -> 
...
'XML11-1ed-Included-char-1-new'(_Config) ->
 ...
 case (xqerl_node:to_xml(xqerl_test:run(case xqerl_node:to_xml(Res) of {xqError,_,_,_,_} -> "deep-equal(<x></x>"; P1 -> "deep-equal(<x>"++P1++"</x>" end ++ " , " ++ "<x>" ++ [60,101,103,103,383,47,62]++ "</x>)" )) == "true" orelse ResXml == Exp) orelse (   ct:fail([[60,115,101,114,105,97,108,105,122,97,116,105,111,110,45,109,97,116,99,104,101,115,32,120,109,108,110,115,61,34,104,116,116,112,58,47,47,119,119,119,46,119,51,46,111,114,103,47,50,48,49,48,47,48,57,47,113,116,45,102,111,116,115,45,99,97,116,97,108,111,103,34,62,38,108,116,59,101,103,103,383,47,38,103,116,59,60,47,115,101,114,105,97,108,105,122,97,116,105,111,110,45,109,97,116,99,104,101,115,62], Res])) of true -> {comment, "any-of"};
   Q -> ct:fail(['any-of', {Res,Exp,Q}]) end.
