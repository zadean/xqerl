    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-fold-right-001.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-fold-right-002'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "array:fold-right([[2,3],[],[4,5,6,7],[6,7],[3]], 0, function($a,$z){array:size($a) + $z})",
    {Env, Opts} = xqerl_test:handle_environment(environment('array', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-fold-right-002.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-fold-right-003'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "array:fold-right([\"the cat\", \"sat\", \"on the mat\"], \"\", function($a,$z){concat($a, \" \", $z)})",
    {Env, Opts} = xqerl_test:handle_environment(environment('array', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-fold-right-003.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-fold-right-004'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "array:fold-right([\"+2\", \"*3\"], \"10\", function($a,$z){concat(\"(\", $z, $a, \")\")})",
    {Env, Opts} = xqerl_test:handle_environment(environment('array', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-fold-right-004.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-fold-right-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-fold-right-005.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-fold-right-006'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "array:fold-right([\" opened the door\", \" went up stairs\"], \"Bob\", function($a,$z){concat($z, $a)})",
    {Env, Opts} = xqerl_test:handle_environment(environment('array', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-fold-right-006.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-fold-right-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-fold-right-007.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-fold-right-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-fold-right-008.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-fold-right-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-fold-right-009.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
