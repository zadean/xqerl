    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-501.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-insert-before-502'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-502.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-insert-before-503'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-503.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-insert-before-504'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-504.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-insert-before-505'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-505.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-insert-before-506'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-506.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-insert-before-507'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-507.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-insert-before-508'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-508.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
            ...
            lists:all(
                ...
                [
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "deep-equal($result[1]?*!local-name(), ('x', 'e', 'f', 'g'))"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "deep-equal($result[2]?*!local-name(), ('e', 'x', 'f', 'g'))"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "deep-equal($result[3]?*!local-name(), ('e', 'f', 'x', 'g'))"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "deep-equal($result[4]?*!local-name(), ('e', 'f', 'g', 'x'))"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
...
'array-insert-before-509'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-509.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-insert-before-510'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-510.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'array-insert-before-511'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "array-insert-before-511.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
