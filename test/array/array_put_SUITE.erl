...
'array-put-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-001.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-002.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-003.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-004.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-005.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-006.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-007.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-008.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
            ...
            lists:all(
                ...
                [
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "deep-equal($result[1]?*!local-name(), ('x', 'f', 'g'))"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "deep-equal($result[2]?*!local-name(), ('e', 'x', 'g'))"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
                    ...
                    case
                        xqerl_test:assert(
                            Res,
                            "deep-equal($result[3]?*!local-name(), ('e', 'f', 'x'))"
                        )
                    of
                        true -> {comment, "Correct results"};
                        {false, F} -> F
...
'array-put-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-009.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-010.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-011.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-012.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'array-put-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "array-put-013.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
