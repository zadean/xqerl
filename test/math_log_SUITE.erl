...
'math-log-004'(_Config) ->
   ...
   Exp = "
            -6.907755278982137e0
        ",
 Tst = xqerl:run("-6.907755278982137e0"),
  ResVal = xqerl_types:value(Res),
  TstVal = xqerl_types:value(Tst),
  if ResVal == TstVal -> {comment, "assert-eq"};
    true -> ct:fail({Res,Exp}) end.
'math-log-005'(_Config) ->
   Qry = "math:log(2)",
   Env = xqerl_test:handle_environment(environment('math')),
   Qry1 = lists:flatten(Env ++ Qry),
   Res = xqerl:run(Qry1),
   ...
   Exp = "
        ...
        ",
 Tst = xqerl:run("0.6931471805599453e0"),
  ResVal = xqerl_types:value(Res),
  TstVal = xqerl_types:value(Tst),
  if ResVal == TstVal -> {comment, "assert-eq"};
    true -> ct:fail({Res,Exp}) end.
'math-log-006'(_Config) ->
   Qry = "math:log(-1)",
   Env = xqerl_test:handle_environment(environment('math')),
   Qry1 = lists:flatten(Env ++ Qry),
   Res = xqerl:run(Qry1),
