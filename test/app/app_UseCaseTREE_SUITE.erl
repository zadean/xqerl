    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "tree-queries-results-q1.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'tree-queries-results-q2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "tree-queries-results-q2.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'tree-queries-results-q3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "tree-queries-results-q3.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'tree-queries-results-q4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "tree-queries-results-q4.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<top_section_count>2</top_section_count>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'tree-queries-results-q5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "tree-queries-results-q5.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'tree-queries-results-q6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "tree-queries-results-q6.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
