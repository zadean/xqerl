...
'fn-substring-before-20'(_Config) ->
   ...
   Exp = "
      ...
      ",
 case xqerl_seq2:size(Res) == 1 andalso  begin Tst1 = xqerl:run("\"1\""),
  ResVal1 = xqerl_types:value(Res),
  TstVal1 = xqerl_types:value(Tst1),
  ResVal1 == TstVal1 end of true -> {comment, "any-of"};
   _ -> ct:fail(['all-of', {Res,Exp}]) end.
...
'fn-substring-before-25'(_Config) ->
   Qry = [115,117,98,115,116,114,105,110,103,45,98,101,102,111,114,101,40,34,65537,65538,65539,34,44,32,34,65538,34,41],
   Qry1 = Qry,
   Res = xqerl:run(Qry1),
   ResXml = xqerl_node:to_xml(Res),
   Options = [{'result',Res}],
