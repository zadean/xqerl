...
'math-pi-001'(Config) ->
   ...
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "math-pi-001.xq"), Qry1),
...
'math-pi-002'(Config) ->
   ...
   Qry1 = lists:flatten(Env ++ Qry),
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "math-pi-002.xq"), Qry1),
             xqerl:run(Mod,Opts) of D -> D catch _:E -> E end,
...
'math-pi-003'(Config) ->
   ...
   Qry1 = lists:flatten(Env ++ Qry),
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "math-pi-003.xq"), Qry1),
             xqerl:run(Mod,Opts) of D -> D catch _:E -> E end,
...
'math-pi-004'(Config) ->
   ...
   Qry1 = lists:flatten(Env ++ Qry),
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "math-pi-004.xq"), Qry1),
             xqerl:run(Mod,Opts) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_eq(Res,"3.141592653589793e0") of 
      true -> {comment, "Equal"};
      {false, F} -> F 
...
'math-pi-005'(Config) ->
   ...
   Qry1 = lists:flatten(Env ++ Qry),
   io:format("Qry1: ~p~n",[Qry1]),
   Res = try Mod = xqerl_code_server:compile(filename:join(__BaseDir, "math-pi-005.xq"), Qry1),
             xqerl:run(Mod,Opts) of D -> D catch _:E -> E end,
   Out =    case xqerl_test:assert_eq(Res,"3.141592653589793e0") of 
      true -> {comment, "Equal"};
      {false, F} -> F 
