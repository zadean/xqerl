    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "XML10-3ed-Mixed-content.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'XML10-4ed-Excluded-char-1-new'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "XML10-4ed-Excluded-char-1-new.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'XML10-4ed-Excluded-char-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "XML10-4ed-Excluded-char-2.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'XML10-5ed-Included-char-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "XML10-5ed-Included-char-1.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
    ...
    Out =
        case xqerl_test:assert_xml(Res, "<foo>&#x7f;</foo>") of
            true -> {comment, "XML Deep equal"};
            {false, F} -> F
        end,
    case Out of
...
'XML11-1ed-Included-char-1-new'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "XML11-1ed-Included-char-1-new.xq"),
                Qry1
            ),
            xqerl:run(Mod)
        of
...
'line-ending-Q001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "line-ending-Q001.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'line-ending-Q002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "line-ending-Q002.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'line-ending-Q003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "line-ending-Q003.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'line-ending-Q007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "line-ending-Q007.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'line-ending-Q008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "line-ending-Q008.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'line-ending-Q009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "line-ending-Q009.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
