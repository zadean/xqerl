...
environment('UCA-collation.en.primary', __BaseDir) ->
    [
        ...
        {params, []},
        {vars, [
            {"collation", "", "'http://www.w3.org/2013/collation/UCA?lang=en;strength=primary;'"}
        ]},
        {namespaces, []},
        {schemas, []},
        {resources, []},
...
'UCA-collation-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-001.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-002.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-003.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-004.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-005.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-006.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-007.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-008.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-010.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-011.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-012.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-013.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-014.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-015.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-016.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-017.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-018'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-018.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-019'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-019.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-020'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-020.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-021'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-021.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-022.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-collation-022a.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-collation-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-023.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-collation-024'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-collation-024.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-reorder-codes-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-001.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-reorder-codes-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-002.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-reorder-codes-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-003.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-reorder-codes-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-004.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-reorder-codes-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-005.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-reorder-codes-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-006.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-reorder-codes-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-007.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-reorder-codes-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-008.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-reorder-codes-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-009.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-reorder-codes-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-010.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-reorder-codes-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-reorder-codes-011.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-params-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-001.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-002.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-003.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-004.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-005.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-006'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-006.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-007'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-007.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-008'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-008.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-009'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-009.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-010'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-010.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-011'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-011.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-012'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-012.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-013.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-014'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-014.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-015'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-015.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-016'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-016.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-017'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-017.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-018'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('pêche','péché',concat($collation,'strength=secondary;backwards=yes;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-018.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-019'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('pêche','péché',concat($collation,'strength=secondary;backwards=no;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-019.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-020'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('pêche','péché',concat($collation,'strength=secondary;normalization=no;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-020.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-021'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('pêche','péché',concat($collation,'strength=secondary;normalization=yes;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-021.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-022'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-022.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-023'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-023.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-024'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-024.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-025'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-025.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-026'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('Epee','épee',concat($collation,'strength=secondary;caseLevel=yes;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-026.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-027'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-027.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-028'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-028.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-029'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-029.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-030'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-030.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-031'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-031.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-032'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-032.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-033'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-033.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-params-034'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-params-034.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-maxVariable-001'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('database','data type',concat($collation,'maxVariable=space;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-001.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-002'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('database','data type',concat($collation,'maxVariable=space;alternate=non-ignorable;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-002.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-003'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('database','data base',concat($collation,'maxVariable=space;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-003.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-004'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('database','data base',concat($collation,'maxVariable=space;alternate=shifted;strength=4;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-004.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-005'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-005.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-006'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('database','data-type',concat($collation,'maxVariable=punct;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-006.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-007'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('database','data-base',concat($collation,'maxVariable=punct;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-007.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-008'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('data base','data-base',concat($collation,'maxVariable=punct;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-008.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-009'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('data base','data-base',concat($collation,'maxVariable=punct;alternate=shifted;strength=4;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-009.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-010'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('data=base','database',concat($collation,'maxVariable=punct;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-010.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-011'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('data=base','database',concat($collation,'maxVariable=symbol;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-011.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-012'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('data=base','data base',concat($collation,'maxVariable=symbol;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-012.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-013'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-013.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-014'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('data=base','data$base',concat($collation,'maxVariable=currency;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-014.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-015'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('$10000','€10000',concat($collation,'maxVariable=currency;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-015.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-maxVariable-016'(Config) ->
    __BaseDir = ?config(base_dir, Config),
    Qry =
        "compare('$10000','€9000',concat($collation,'maxVariable=currency;numeric=yes;alternate=shifted;fallback=no'))",
    {Env, Opts} = xqerl_test:handle_environment(environment('UCA-collation.en', __BaseDir)),
    Qry1 = lists:flatten(Env ++ Qry),
    io:format("Qry1: ~p~n", [Qry1]),
    Res =
        try
            Mod = xqerl_code_server:compile(
                filename:join(__BaseDir, "UCA-maxVariable-016.xq"),
                Qry1
            ),
            xqerl:run(Mod, Opts)
        of
...
'UCA-misc-lang-001'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-misc-lang-001.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-misc-lang-002'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-misc-lang-002.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-misc-lang-003'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-misc-lang-003.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
...
'UCA-misc-lang-004'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "UCA-misc-lang-004.xq"), Qry1),
            xqerl:run(Mod, Opts)
        of
            D -> D
        catch
