...
'annex-1'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "annex-1.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "some data") of
...
'annex-2'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "annex-2.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'annex-3'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "annex-3.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'annex-4'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "annex-4.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'annex-5'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "annex-5.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'annex-6'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "annex-6.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
...
'annex-7'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "annex-7.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
    ...
    Out =
        case xqerl_test:assert_string_value(Res, "A StringA StringA String") of
            true -> {comment, "String correct"};
            {false, F} -> F
        end,
    case Out of
...
'annex-8'(Config) ->
    ...
    Res =
        try
            Mod = xqerl_code_server:compile(filename:join(__BaseDir, "annex-8.xq"), Qry1),
            xqerl:run(Mod)
        of
            D -> D
        catch
