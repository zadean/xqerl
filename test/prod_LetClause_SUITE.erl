...
'LetExpr016'(_Config) ->
   ...
   Exp = "
      ...
      ",
   case catch xqerl_node:to_xml(xqerl_test:run(case xqerl_node:to_xml(Res) of {xqError,_,_,_,_} -> "deep-equal(<x></x>"; P -> "deep-equal(<x>"++P++"</x>" end ++ " , " ++ "<x>"++"<root><elem1/><elem2/><elem3 att=\"test\"/></root>"++"</x>)")) == "true" of
           true -> {comment, "assert-xml"};
           _ -> 
...
'LetExpr017'(_Config) ->
   ...
   Exp = "
         <a><b><c> 123 </c></b></a>
      ",
   case catch xqerl_node:to_xml(xqerl_test:run(case xqerl_node:to_xml(Res) of {xqError,_,_,_,_} -> "deep-equal(<x></x>"; P -> "deep-equal(<x>"++P++"</x>" end ++ " , " ++ "<x>"++"<a><b><c> 123 </c></b></a>"++"</x>)")) == "true" of
           true -> {comment, "assert-xml"};
           _ -> 
...
'LetExpr019'(_Config) ->
   ...
   Exp = "
         <elem/><elem2/>
      ",
   case catch xqerl_node:to_xml(xqerl_test:run(case xqerl_node:to_xml(Res) of {xqError,_,_,_,_} -> "deep-equal(<x></x>"; P -> "deep-equal(<x>"++P++"</x>" end ++ " , " ++ "<x>"++"<elem/><elem2/>"++"</x>)")) == "true" of
           true -> {comment, "assert-xml"};
           _ -> 
